...
MTK_TEST_MAIN_BEGIN
     ...
     * gap, a contiguous one is not. ------------ */
    {
        ...
        mtk_spi_native_header_t c3 = c1;
        c3.packet_seq = 551; /* contiguous with c2 -- no new gap */
        mtek_spi_native_dispatch_feed_cell(&dctx, &c3, NULL, 9, &resp_hdr, resp_payload, &resp_len);
