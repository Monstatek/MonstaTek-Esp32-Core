 ...
 *     to the SAME handler), and TIME_SYNC_START's native/Legacy SPI Compatibility capability
 ...
 *     MTK_CAP_SUPPORTED for both the native and Legacy SPI Compatibility profiles, but this
 ...
 * test_async_service_safety.c's own established pattern) -- every
 ...
 * see test_session_publication.c's own doc comment on
 ...
 * test_async_service_safety.c's own won-hook pause (there tied to
 ...
 * test_async_service_safety.c's own. ---- */
 ...
 * the class match, not class alone -- see test_session_publication.c's
...
/* Direct (mtk_test_call-based) GATT helpers -- see
 * test_async_service_safety.c's doc comment on why the GATT vendor-handle-reuse
 ...
 * the production capability to UNSUPPORTED (native + compat_spi; factory_uart
