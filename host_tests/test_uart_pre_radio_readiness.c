/* "the actual UART command-processing loop is still created only after the same
 * unconditional Wi-Fi/NimBLE initialization span... RC12 can falsely announce
 * readiness and then fail the mode command exactly as RC11 did." app_main.c now
 * creates uart_repl_task (and wires mtk_core_init/
 ...
 * reordering -- it does NOT exercise a real UART peripheral (no target access
 * from this host), only the same production mtek_uart_adapter.c/
 * mtek_router.c/mtek_transport_select.c logic app_main.c actually calls, driven
 * through the identical minimal bring-up sequence (deliberately OMITTING every
 * service register/set_hal call) to prove two things mechanically rather than by
 * inspection alone:
 *
 * 1. Bare CRLF (line_is_recognized("") == 0, matching app_main's own "never even
 * attempt process_line" bare-Enter path) and every global command the STM32's
 * own frozen readiness probe actually sends ("mode -w"/"mode -b"/"mode") answer
 * with their exact, correct wire text using ONLY mtk_core_init/mtk_arbiter_init/
 * mtk_router_init/mtk_transport_claim_set_lock/mtk_transport_claim_ reset --
 * zero wifi/ble/system/capture service or HAL dependency.
 *
 * 2. A command that DOES need the router (a WIFI-mode opcode, "scan -a") issued
 * in this same pre-registration state degrades to its own pre-existing,
 * already-shipped honest failure text ("[!] Scan failed.\n" --
 * mtek_uart_adapter.c's own existing cap.status != MTK_STATUS_ACCEPTED branch,
 * reached via mtk_router.c's find_service == NULL -> MTK_STATUS_UNSUPPORTED
 * path) -- never a crash, a hang, or malformed output, exactly the same
 * honest-refusal contract this tree already relies on for
 * wifi_hal_ready==0/ble_hal_ready==0. */
#include "mtk_test.h"
#include "mtek_core.h"
