...
/* ---- Lock domains, mirroring test_service_lifecycle_safety.c's own
 ...
 * blocking on the real mutex, exactly mirroring test_session_publication.c's
 * pub_lock_fn instrumentation. Lets a
 ...
 * test_service_lifecycle_safety.c's own.
 *
 ...
 * test_async_service_safety.c and test_service_lifecycle_safety.c (native SPI dispatch is not
 ...
 * see test_service_lifecycle_safety.c's own doc comment on this exact
...
static void test_capture_channel_hop_pre_hal_race_closed(void) {
     ...
     * here, exactly mirroring test_session_publication.c's
     * "join the dispatching trigger while the real worker
