/* The test-only asynchronous fixture.
 *
 ...
 * (deferred-worker pool budgeting, NO_MEMORY-on-overflow, eviction/ABA, cancel,
 * publish-guard/reset races). They historically borrowed the real
 * TIME_SYNC_START opcode for this, which forced its production capability to
 * stay SUPPORTED despite this candidate having no SNTP client. This fixture
 * provides an equivalent opcode that lives ONLY in the test-only opcode overlay
 * (mtek_opcode_overlay.h) -- never in the production table or GET_CAPABILITIES
 * -- so TIME_SYNC_START can now advertise the truthful UNSUPPORTED.
 *
 ...
 * no_radio_lease, the same request/response descriptors and deadline envelope)
 * and is routed by mtek_system_dispatch to the SAME production handler
 * (handle_time_sync_start), so a test using it drives the exact production async
 * path -- not a divergent reimplementation. It lives in system service 0x0000
 * under a reserved test opcode number (0x00F0) that no production opcode uses.
 *
 ...
 * inline. mtk_test_bootstrap (mtk_test_bootstrap.h) is unchanged; a test that
 * wants the fixture calls mtk_test_async_fixture_install after its own
 * core/router/service setup and mtk_test_async_fixture_uninstall (or relies on
 * process exit) in teardown. */
#pragma once
...
/* The paired test-only generic SYNCHRONOUS STOP (0x00F1), routed by
 * mtek_system_dispatch (under MTK_ENABLE_TEST_OPCODES, never compiled into the
 * ESP32 target) to the SAME production handle_time_sync_stop. Provided so tests
 * that need a generic token- addressed STOP no longer depend on the real
 * TIME_SYNC_STOP opcode, which is now UNSUPPORTED on native. Reuses the
 * TIME_SYNC_STOP descriptors, so a test encoding a mtk_time_sync_stop_req_t
 * against op->req_desc is unchanged. handle_time_sync_stop family-validates a
 * 0x00F1 STOP against the test START family (0x00F0), so it correctly stops
 * tokens minted by g_mtk_test_async_op above. */
