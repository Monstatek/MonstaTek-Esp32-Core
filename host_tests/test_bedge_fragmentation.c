/* Bedge/C3 dispatch-layer OUTBOUND fragmentation: a response too large
 * for one 502-byte cell is staged and drained as FRAG cells followed by
 * one terminal RESP/NAK cell (mtek_bedge_dispatch_poll_outbound), mirroring
 * mtk_bedge_reassembly_feed's own inbound convention exactly. Exercises
...
static void stage(mtk_bedge_dispatch_ctx_t *dctx, uint16_t msg_id, uint8_t final_type, const uint8_t *data, uint16_t len) {
...
static void drain_and_check(mtk_bedge_dispatch_ctx_t *dctx, uint16_t expect_msg_id, uint8_t expect_final_type,
                             const uint8_t *expect_data, uint16_t expect_len) {
    uint8_t reassembled[MTK_BEDGE_MAX_REASSEMBLY_PAYLOAD];
    uint16_t got = 0;
    ...
    for (unsigned guard = 0; guard < 32; guard++) {
        ...
        MTK_CHECK_EQ(resp_hdr.magic, MTK_BEDGE_MAGIC);
        MTK_CHECK_EQ(resp_hdr.msg_id, expect_msg_id);
        if (resp_hdr.msg_type == MTK_BEDGE_MSG_FRAG) {
            frag_cells++;
            MTK_CHECK_EQ(resp_len, MTK_BEDGE_SINGLE_CELL_PAYLOAD_MAX); /* never a short FRAG */
    ...
    unsigned expect_frags = expect_len > MTK_BEDGE_SINGLE_CELL_PAYLOAD_MAX
        ? (expect_len - 1) / MTK_BEDGE_SINGLE_CELL_PAYLOAD_MAX
        : 0;
...
MTK_TEST_MAIN_BEGIN
    ...
    static uint8_t pattern[MTK_BEDGE_MAX_REASSEMBLY_PAYLOAD];
    for (unsigned i = 0; i < sizeof(pattern); i++) pattern[i] = (uint8_t)(i * 7 + 3);

    /* Polling with nothing staged: well-formed IDLE, not garbage. */
    {
        mtk_bedge_header_t resp_hdr; uint8_t resp_payload[MTK_BEDGE_SINGLE_CELL_PAYLOAD_MAX]; uint16_t resp_len = 0xFFFF;
        mtek_bedge_dispatch_poll_outbound(&dctx, &resp_hdr, resp_payload, &resp_len);
    ...
    stage(&dctx, 0x1001, MTK_BEDGE_MSG_RESP, pattern, 0);
    drain_and_check(&dctx, 0x1001, MTK_BEDGE_MSG_RESP, pattern, 0);

    /* 1 byte: single terminal cell. */
    stage(&dctx, 0x1002, MTK_BEDGE_MSG_RESP, pattern, 1);
    drain_and_check(&dctx, 0x1002, MTK_BEDGE_MSG_RESP, pattern, 1);

    /* Exactly 502 bytes: still fits in one terminal cell, no FRAG at all. */
    stage(&dctx, 0x1003, MTK_BEDGE_MSG_NAK, pattern, MTK_BEDGE_SINGLE_CELL_PAYLOAD_MAX);
    drain_and_check(&dctx, 0x1003, MTK_BEDGE_MSG_NAK, pattern, MTK_BEDGE_SINGLE_CELL_PAYLOAD_MAX);

    /* 503 bytes: one full 502-byte FRAG cell + one 1-byte terminal cell. */
    stage(&dctx, 0x1004, MTK_BEDGE_MSG_RESP, pattern, MTK_BEDGE_SINGLE_CELL_PAYLOAD_MAX + 1);
    drain_and_check(&dctx, 0x1004, MTK_BEDGE_MSG_RESP, pattern, MTK_BEDGE_SINGLE_CELL_PAYLOAD_MAX + 1);

    /* Full 4082-byte reassembly ceiling: 8 full FRAG cells + one terminal
     * cell carrying the 4082 - 8*502 = 66-byte remainder. */
    stage(&dctx, 0x1005, MTK_BEDGE_MSG_RESP, pattern, MTK_BEDGE_MAX_REASSEMBLY_PAYLOAD);
    drain_and_check(&dctx, 0x1005, MTK_BEDGE_MSG_RESP, pattern, MTK_BEDGE_MAX_REASSEMBLY_PAYLOAD);
    MTK_CHECK_EQ((MTK_BEDGE_MAX_REASSEMBLY_PAYLOAD - 1) / MTK_BEDGE_SINGLE_CELL_PAYLOAD_MAX, 8);
     ...
     * the framing layer already (test_bedge_framing.c); here we prove the
     * outbound struct itself never lets sent_offset exceed total_len even
     * under a pathological repeated-poll sequence. */
    stage(&dctx, 0x1006, MTK_BEDGE_MSG_RESP, pattern, 600);
    for (unsigned i = 0; i < 10; i++) {
        mtk_bedge_header_t resp_hdr; uint8_t resp_payload[MTK_BEDGE_SINGLE_CELL_PAYLOAD_MAX]; uint16_t resp_len = 0;
        mtek_bedge_dispatch_poll_outbound(&dctx, &resp_hdr, resp_payload, &resp_len);
        MTK_CHECK(dctx.outbound.sent_offset <= dctx.outbound.total_len);
        if (!dctx.outbound.active) { MTK_CHECK_EQ(resp_hdr.msg_type, i == 1 ? MTK_BEDGE_MSG_RESP : MTK_BEDGE_MSG_IDLE); }
    }
