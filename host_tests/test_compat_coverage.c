 ...
 * capability_state.compat_spi=SUPPORTED is actually reachable through its
 * real, documented Legacy SPI Compatibility wire opcode -- not merely present in a
 * registry, and never silently answered by the generic
 * default-unsupported switch case (Part 1, table-driven). Part 2 goes
 * further for every opcode with a confirmed exact Legacy SPI Compatibility wire byte
 * layout: proves real request decode (the fake HAL/service actually
 * receives the decoded fields, not just "some request"), a real
 * canonical side effect, and the exact confirmed Legacy SPI Compatibility response byte
...
MTK_TEST_MAIN_BEGIN
    ...
    const cov_row_t rows[] = {
         ...
         * -- its compat_spi capability is now UNSUPPORTED (no SNTP client),
         * so it is no longer one of the "opcodes schemas.json marks
         * capability_state.compat_spi=SUPPORTED" this table enumerates. */
    ...
    /* AP_SCAN_START: real fake-HAL AP records translated into Legacy SPI Compatibility's
     ...
     * translated into Legacy SPI Compatibility's confirmed exact [count:2] +
     * [mac:6][rssi:1] shape. */
    {
        mtk_fake_wifi_reset();
        mtek_wifi_set_hal(&g_fake_wifi_hal);
         ...
         * bssid/channel/duration this Legacy SPI Compatibility payload encoded. */
     ...
     * matching Legacy SPI Compatibility's own confirmed "does not interpret the cookie at
     ...
     * capability gate), never a hand-rolled Legacy SPI Compatibility-layer NAK. */
    {
