...
MTK_TEST_MAIN_BEGIN
    ...
    MTK_CHECK_EQ(MTK_OPCODE_COUNT, 113); /* 94 canonical + 6 ESP-NOW + 13 IEEE 802.15.4 (7 radio + 3 RCP + 3 capture) */
    ...
    MTK_CHECK_EQ(i154_count, 13);
    MTK_CHECK_EQ(sys_count, 10);
    ...
    MTK_CHECK_EQ(deauth_start->cap_compat_spi, MTK_CAP_SUPPORTED);

    /* STA_CONNECT: factory-UART UNAVAILABLE (no connect verb exists in the
     * audited console grammar), native/Legacy SPI Compatibility SUPPORTED. */
    ...
    MTK_CHECK_EQ(sta_connect->cap_compat_spi, MTK_CAP_SUPPORTED);
