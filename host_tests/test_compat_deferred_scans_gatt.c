/* RC12 final blocker correction, item 2 "real deferred compatibility execution":
 * DETERMINISTIC proof (forced condvar synchronization -- never timing-only
 * sleeps) that the Legacy SPI Compatibility adapter's two-part response/event state
 ...
 * unsolicited compatibility wire event or new opcode is introduced. */
#include "mtk_test.h"
...
MTK_TEST_MAIN_BEGIN
     ...
     * confirmed final Compatibility response. This is the response-only initial-
    ...
    /* -- 8a: DEFERRED terminal TIMEOUT (case1 -> case2 -> terminal failure). */
    {
        ...
        MTK_CHECK_EQ(rp[0], MTK_COMPAT_STATUS_ERR_TIMEOUT);  /* existing canonical-to-compatibility mapping */
