/* Bedge/C3 `m1_link` framing (001-profile-bootstrap-feasibility.md Sec 1):
...
MTK_TEST_MAIN_BEGIN
    ...
    MTK_CHECK_EQ(mtk_bedge_crc16((const uint8_t *)"123456789", 9), 0x29B1);
    ...
    hdr.msg_type = MTK_BEDGE_MSG_IDLE; hdr.msg_id = 0; hdr.payload_len = 0;
    uint8_t cell[MTK_BEDGE_CELL_SIZE];
    mtk_bedge_build_cell(&hdr, NULL, cell);
    MTK_CHECK_EQ(cell[0], 0x31); MTK_CHECK_EQ(cell[1], 0x4D); /* 0x4D31 LE on the wire */

    mtk_bedge_header_t parsed; const uint8_t *payload;
    MTK_CHECK_EQ(mtk_bedge_parse_cell(cell, &parsed, &payload), MTK_BEDGE_PARSE_OK);
    MTK_CHECK_EQ(parsed.msg_type, MTK_BEDGE_MSG_IDLE);

    /* Malformed input rejected at every field. */
    uint8_t bad[MTK_BEDGE_CELL_SIZE];
    memcpy(bad, cell, sizeof(bad)); bad[0] ^= 0xFF;
    MTK_CHECK_EQ(mtk_bedge_parse_cell(bad, &parsed, &payload), MTK_BEDGE_PARSE_BAD_MAGIC);
    memcpy(bad, cell, sizeof(bad)); bad[2] = 0x02;
    MTK_CHECK_EQ(mtk_bedge_parse_cell(bad, &parsed, &payload), MTK_BEDGE_PARSE_BAD_VERSION);
    memcpy(bad, cell, sizeof(bad)); bad[3] = 0x0F; /* unknown msg_type */
    MTK_CHECK_EQ(mtk_bedge_parse_cell(bad, &parsed, &payload), MTK_BEDGE_PARSE_BAD_MSG_TYPE);
    memcpy(bad, cell, sizeof(bad)); bad[MTK_BEDGE_HEADER_SIZE] ^= 0xFF; /* corrupt CRC-covered byte */
    hdr.payload_len = 1;
    /* build a frame with 1 payload byte to have something to corrupt */
    uint8_t onebyte = 0xAA;
    uint8_t cell2[MTK_BEDGE_CELL_SIZE];
    mtk_bedge_build_cell(&hdr, &onebyte, cell2);
    uint8_t badcrc[MTK_BEDGE_CELL_SIZE]; memcpy(badcrc, cell2, sizeof(badcrc));
    badcrc[MTK_BEDGE_HEADER_SIZE] ^= 0xFF;
    MTK_CHECK_EQ(mtk_bedge_parse_cell(badcrc, &parsed, &payload), MTK_BEDGE_PARSE_BAD_CRC);

    MTK_CHECK_EQ(mtk_bedge_parse_bounded(cell, 3, &parsed, &payload), MTK_BEDGE_PARSE_TRUNCATED);

    /* Cross-profile rejection: native's fixed magic never parses as Bedge. */
    uint8_t native_ish[MTK_BEDGE_CELL_SIZE] = {0};
    native_ish[0] = 0x4D; native_ish[1] = 0x31; native_ish[2] = 0x53; native_ish[3] = 0x31;
    MTK_CHECK_EQ(mtk_bedge_parse_bounded(native_ish, sizeof(native_ish), &parsed, &payload), MTK_BEDGE_PARSE_BAD_MAGIC);
    MTK_CHECK_EQ(mtk_bedge_try_recognize_discovery_frame(native_ish, sizeof(native_ish)), 0);
    MTK_CHECK_EQ(mtk_bedge_try_recognize_discovery_frame(cell, sizeof(cell)), 1);

    /* Fragmentation/reassembly: FRAG then terminal REQ completes the message. */
    mtk_bedge_reassembly_t ctx;
    mtk_bedge_reassembly_reset(&ctx);
    mtk_bedge_header_t frag = {0};
    frag.magic = MTK_BEDGE_MAGIC; frag.version = MTK_BEDGE_VERSION; frag.msg_type = MTK_BEDGE_MSG_FRAG;
    frag.msg_id = 7; frag.payload_len = 5;
    uint8_t part1[5] = {1,2,3,4,5};
    MTK_CHECK_EQ(mtk_bedge_reassembly_feed(&ctx, &frag, part1), MTK_BEDGE_REASM_IN_PROGRESS);
    MTK_CHECK_EQ(ctx.len, 5);
    mtk_bedge_header_t term = frag; term.msg_type = MTK_BEDGE_MSG_REQ; term.payload_len = 3;
    uint8_t part2[3] = {6,7,8};
    MTK_CHECK_EQ(mtk_bedge_reassembly_feed(&ctx, &term, part2), MTK_BEDGE_REASM_COMPLETE);
    MTK_CHECK_EQ(ctx.len, 8);
    for (int i = 0; i < 8; i++) MTK_CHECK_EQ(ctx.data[i], i + 1);

    /* Overflow rejected, not silently truncated. */
    mtk_bedge_reassembly_reset(&ctx);
    ctx.active = 1; ctx.msg_id = 1; ctx.len = MTK_BEDGE_MAX_REASSEMBLY_PAYLOAD;
    mtk_bedge_header_t small = {0}; small.msg_type = MTK_BEDGE_MSG_FRAG; small.msg_id = 1; small.payload_len = 1;
    uint8_t onemore = 0;
    MTK_CHECK_EQ(mtk_bedge_reassembly_feed(&ctx, &small, &onemore), MTK_BEDGE_REASM_OVERFLOW);

    /* ---- End-to-end dispatch: DEAUTH_START via real Bedge wire bytes ---- */
    ...
    mtk_bedge_header_t unknown_hdr = req_hdr; unknown_hdr.msg_id = 0x9999; unknown_hdr.payload_len = 0;
    mtek_bedge_dispatch_request(&dctx, &unknown_hdr, req_payload, &resp_hdr, resp_payload, &resp_len);
