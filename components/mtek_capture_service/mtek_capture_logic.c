 ...
 * dependency, host-testable; reuses mtek_wifi_hal_t's promiscuous-mode
 * primitives (mtek_wifi_get_hal) since MonstaShark capture and WPA handshake
 * capture share the same underlying radio resource (RADIO_OWNER_WIFI, arbiter
 * class M), never active together. */
...
/* Capture token family (service 0x0004), every token minted by
 * CAPTURE_START (0x0001). Passed to mtk_op_*_family so a non-capture token
 * handed to a capture STOP/STATUS/session-info/stats handler is rejected
 * NOT_FOUND with no side effect. */
...
/* See this function's own declaration doc comment (mtek_capture_service.h) for
 * the full rationale. A DEDICATED mutex (not the one shared core/arbiter/router/
 * async-queue registration use elsewhere in this tree), specifically so frame_cb
 * can safely call emit_stream/emit_event while still holding this lock without
 * any risk of a recursive acquisition on a non- recursive FreeRTOS mutex --
 * emit_stream/emit_event ultimately reach a DIFFERENT mutex (the async_queue's
 * own, for a queue-backed sink) or none at all (a stack-local sink), never this
 * one. */
...
/* A dedicated action/lease lock, genuinely distinct from s_cap_lock/s_cap_unlock
 * above -- see its own doc comment (mtek_capture_service.h's
 * mtek_capture_set_action_lock) for the full rationale. Only
 * mtek_capture_channel_hop_tick and handle_capture_start ever acquire it; never
 * frame_cb, never anything a blocking HAL call could re-enter, so it is safe to
 * hold across the real hal->set_channel round-trip. */
...
/* See mtek_capture_service.h's own doc comment on
 * mtek_capture_service_mark_tick_task_failed. Ready by default; app_main.c marks
 ...
 * as mtek_wifi_logic.c's s_deauth/s_hs (the struct doc comments).
 * frame_cb/hop_tick re-resolve to a live record via mtk_op_find each time;
 * handlers keyed on a caller- supplied token compare it against s_cap.token
 * before trusting any of this struct's other fields, never assuming a fresh
 * mtk_op_find match means "this is MY session". */
static struct {
    uint32_t token;
    ...
    uint32_t boot_epoch;
    /* mtk_request_ctx_t's own session_generation field (0 for every
     * non-native-SPI adapter -- never fenced), captured once at
     * handle_capture_start time. frame_cb and mtek_capture_channel_hop_tick --
     * both of which run long after this call returns, from contexts with no
     * request ctx of their own -- validate against THIS stored value via
     * mtk_op_begin_publish_guard, never a caller's own ctx. */
    uint32_t session_generation;
    mtk_request_ctx_t ctx;
    uint8_t mode; /* 0=PUSH, 1=POLL */
    uint16_t snap_len;
    uint32_t started_boot_us;
    /* The requested session duration (0 = run until explicitly stopped, matching
     * every other List B operation's own count=0/duration=0 "unbounded"
     * convention in this tree) and the wall-clock start time (now_ms units,
     * distinct from started_boot_us's own boot-relative microsecond field above)
     * needed to enforce it -- checked by mtek_capture_channel_hop_tick below
     * (the periodic driver, despite the hop-specific name; renamed in its own
     * doc comment, not its symbol, to avoid an unrelated header churn). */
     ...
     * hal->promisc_start), invoked directly from the Wi-Fi driver's own internal
     * context -- a stack budget this project does not configure and cannot
     * assume is as generous as its own application tasks'
     * (spi_runtime_task/uart_repl_task, 12,288 bytes each). Real ELF measurement
     * (-fstack-usage) showed frame_cb's own frame at 3,008 bytes, dominated by
     * three per-call locals: a full capture_record_t (~1,020 bytes,
     * `data[1000]`), and two 952-byte PUSH-mode STREAM fragment-header/chunk
     * buffers. All three move here -- frame_cb is already strictly
     * single-threaded per invocation (serialized under cap_lock for the record,
     * and the STREAM-building section runs only after cap_lock is released but
     * still only from this one callback, never re-entrantly with itself) --
     * eliminating essentially all of frame_cb's own stack footprint rather than
     * merely relocating one piece of it. */
    capture_record_t frame_scratch;
    uint8_t stream_chunk1[8 + 944];
    uint8_t stream_chunk2[8 + 944];
} s_cap;

/* Hand-written field table for CAPTURE_POLL_READ's RECORD variant only: the
 * tagged-union response shape (EMPTY transmits zero body bytes) isn't
 * representable by the generic generated-struct codec, so this one opcode's
 * response is assembled directly (mtek_core.h emit_response_raw) instead of
 * forcing a one-off extension onto the schema generator for a single message. */
...
/* "capture filters... are stored but not applied". A set BSSID filter matches
 * any of the three 802.11 header address fields (addr1/addr2/ addr3, the same
 * convention mtek_wifi_hal_esp32.c's own station-target scan already uses) so a
 * filtered session captures traffic FROM, TO, or belonging to the target AP's
 * BSS, not just one specific direction. */
...
static void frame_cb(void *user, const uint8_t *frame, uint16_t len, int8_t rssi, uint8_t channel) {
    /* `user` is now this exact registration's own immutable token, captured once
     * at handle_capture_start's own promisc_start call -- previously always
     * NULL, carrying no identity of its own at all. See hs_frame_cb's own doc
     * comment (mtek_wifi_ logic.c) for the full rationale: esp32_promisc_service
     * can legitimately copy (cb, user) together, under its own mutex, an instant
     * BEFORE a concurrent STOP runs -- and a brand-new CAPTURE_START can then
     * reinitialize s_cap for a wholly different operation before this
     * already-copied, now-stale call actually executes. Reading
     * s_cap.token/boot_epoch fresh (an earlier design) would validate this stale
     * call against the NEW session's own identity instead of refusing it. */
    uint32_t registered_token = (uint32_t)(uintptr_t)user;
    /* This snapshot was previously read completely UNLOCKED -- a genuine data
     * race against cap_lock-protected writers (handle_capture_start's own
     * reinit, capture_teardown). */
    cap_lock();
    ...
    /* The op-table check above only proves was not YET finalized at that exact
     * instant -- a concurrent peer-session reset (which itself finalizes an
     * active capture via mtek_capture_cancel_active_for_peer_reset -> capture_
     * teardown) can still land in the window between that check and the
     * shared-state publish (ring/counters) and queue emission (emit_stream)
     * below. mtk_op_begin_publish_guard, acquired here and held across ALL of
     * that -- never across any HAL call, per its own "final validation,
     * shared-state publication, and queue emission only" contract -- closes the
     * window. Skipped entirely (no state touched at all) if the session has
     * already gone stale by this point. */
    if (!mtk_op_begin_publish_guard(session_generation)) return;

    cap_lock();
    /* Re-validate the token identity again, immediately before mutating any
     * shared state -- the window between the checks above and this lock
     * acquisition is small but real. */
    ...
    /* s_cap.frame_scratch, not a stack-local -- see s_cap's own doc comment.
     * Safe to read again below AFTER cap_unlock (same established pattern this
     * function already uses for `sink`/`token`): frame_cb is invoked strictly
     * one frame at a time by the Wi-Fi driver, never concurrently with itself,
     * so nothing else can overwrite this scratch record in between. */
    ...
    /* Every captured frame -- PUSH or POLL -- now also lands in the shared
     * bounded ring (drop-oldest on overflow, same policy either mode already
     * used on its own), so a POLL_READ issued mid-PUSH- session (or after a PUSH
     * session stalls on missing credit) can still retrieve recently-captured
     * frames instead of nothing. PUSH mode's own credit-gated STREAM delivery
     * below is unaffected -- this is in ADDITION to it, not instead of it. */
    ...
    /* PUSH: credit-gated STREAM delivery, fragmented per (25-byte first-fragment
    ...
    if (s_cap.credit_bytes < needed) {
        /* "drops a frame immediately when credit is absent" -- the frame is NOT
         * actually lost any more (it already landed in the common ring above,
         * retrievable via POLL_READ); only the STREAM delivery itself is skipped
         * for lack of credit. credit_stalls still counts this real condition for
         * diagnostics. dropped_frames is intentionally NOT double-incremented
         * here -- the ring-overflow branch above is the only place that
         * increments it, matching its own single, well-defined meaning ("a frame
         * could not retain anywhere"), not "a frame PUSH couldn't stream this
         * instant". */
        s_cap.credit_stalls++;
        cap_unlock();
        mtk_op_end_publish_guard();
    ...
    /* s_cap.stream_chunk1/2, not stack-locals -- see s_cap's own doc comment.
     * Zero-init preserved exactly as before (byte 24 reserved/padding -- must
     * never carry stale memory onto the wire; a persistent field could carry a
     * stale value from a PRIOR frame's own write if not re-zeroed every call, so
     * the explicit memset below is required here, unlike a stack-local that
...
/* (the overflow half -- the synchronization half is the caller's own
 * responsibility now that native SPI's CREDIT class calls this from the same
 * locked dispatch path as every other shared-state mutation,
 * mtek_spi_native_dispatch.c) + (native SPI's own new CREDIT handler needs to
 * know whether the grant was actually applied to report an honest wire status,
 * not just fire-and-forget): saturating add (never wraps past UINT32_MAX) and a
 * real applied/not-applied return, matching this service's own
 * "session-token-scoped, explicit status" requirement for the new native SPI
 * CREDIT class. Returns 1 if `token` matched the currently active capture
 * session (credit applied), 0 otherwise (unknown/mismatched/no active session --
 * a safe no-op, not an error, matching the pre-existing UART/Mtek Compatibility
 * callers' own fire-and-forget usage of this same function). */
...
/* Forward declaration: handle_capture_start's own new promisc_start failure path
 * needs this; full definition (and its own doc comment) is further down, also
 * called from handle_capture_stop and mtek_capture_channel_hop_tick.
 *
 * Takes (token, boot_epoch) rather than a retained `mtk_operation_record_t *` --
 * mtek_capture_channel_hop_ tick's own caller context only ever has a locked
 * mtk_op_snapshot's token/epoch fields to offer in the first place (it must
 * never retain a live pointer across its own periodic-tick call boundary -- the
 * same ABA-hazard rationale as s_deauth/s_hs elsewhere in this tree), and
 * handle_capture_start/_stop's own already-valid token/epoch pair is exactly as
 * safe to pass here directly, with no pointer dereference inside this function
 * needed at all. */
...
static void handle_capture_start(mtk_request_ctx_t *ctx, const mtk_opcode_entry_t *op,
    ...
    /* A nonzero duration (auto-stop) or hop-mode channel plan is only ever
     * enforced by mtek_capture_channel_ hop_tick, itself only ever called from
     * main/app_main.c's own ble_tick_task -- refuse honestly, before acquiring
     * any resource, if that task never started. An unbounded, non-hopping
     * capture needs no tick at all (frame_cb alone drives it) and is unaffected. */
     ...
     *
     * This file's own global nested-lock order is `cap_action_lock -> pub_lock
     * -> cap_lock` -- exactly mtek_capture_channel_hop_tick's own established
     * order (cap_action_ lock at its own top, mtk_op_begin_publish_guard i.e.
     * pub_lock next, cap_lock innermost). The action lease is acquired HERE,
     * before the admission guard even begins, so this call can never hold
     * pub_lock while waiting for the action lease that a paused hop tick already
     * holds -- the previous order (pub_lock first, action lease second) was the
     * reverse of hop_tick's own and formed the deadlock's other half. See
     * mtek_core.h's own admission-guard doc comment for the full nested-lock
     * contract this establishes for every Core path. */
    cap_action_lock();
     ...
     * response and the promisc_start call below (which can synchronously deliver
     * frames via frame_cb on the fake HAL, or race a real target's own
     * callback). */
    ...
    /* s_cap is shared, concurrently-read singleton state -- frame_cb (a real
     * Wi-Fi driver RX callback that can start firing the instant
     * hal->promisc_start below arms it, possibly before this function even
     * returns) and mtek_capture_channel_hop_tick (a separate, periodic tick
     * context) both read it under cap_lock. Initializing it here without the
     * same lock let either of those observe a partially-written struct -- a
     * real, not merely theoretical, data race. Locked for the whole init,
     * matching handle_ap_scan_start/etc elsewhere in this tree that mutate
     * shared session state before anything else can observe it. */
    uint8_t fixed_channel = (req.channel_plan.mode == 0) ? req.channel_plan.channel : 1;
    /* The action lease (acquired above, before the admission guard) is what
     * actually excludes a hop tick's own HAL action/commit for the whole span
     * from here through cap_unlock below -- if a hop tick for the session this
     * call is about to replace is currently mid-action (already past its own
     * final identity validation, about to call or still inside
     * hal->set_channel), this reinit genuinely cannot proceed until that tick's
     * own action fully completes (commits or discovers it is stale and bails)
     * and releases the SAME lease, so this reinit can never race a HAL
     * round-trip already in flight for the session being replaced. The lease is
     * released right after cap_unlock below -- never held across the ACCEPTED
     * response, mtk_op_end_admission_guard, or this function's own later
     * hal->promisc_start call, none of which need it. */
    cap_lock();
    ...
    /* Real hop-mode state (mode==1) -- mtek_capture_channel_hop_tick below
     * actually switches channel once hop_dwell_ms elapses, driven by a
     * target-side periodic tick (matching mtek_ble_signal_meter_
     * tick/mtek_ble_gatt_tick's own established pattern -- see
     * mtek_capture_service.h). ESP32-C6 is 2.4GHz-only, matching every other
     * opcode in this tree that rejects band==1/5GHz, so hopping cycles the 13
     * 2.4GHz channels regardless of channel_plan.band. */
    ...
    /* The accept response above is already sent by this point (an ACCEPTED_ASYNC
     * operation's own established shape -- "accepted" means the request was
     * understood and an operation object created, not that the underlying radio
     * operation is guaranteed to succeed), so a promisc_start failure here
     * cannot retroactively change it -- but it must not be silently discarded
     * either, leaving a capture session marked RUNNING forever with no real
     * frame delivery ever possible. Torn down immediately (same cleanup path a
     * real STOP would run) with a real terminal event and status, exactly as if
     * it had failed moments after a successful start rather than never having
     * truly started at all. */
    const mtk_wifi_hal_t *hal = mtek_wifi_get_hal();
    /* `user` is this exact registration's own immutable token, not NULL -- see
     * frame_cb's own doc comment for the full ABA-hazard rationale. */
...
/* Test-only pause hook -- see its own doc comment in mtek_capture_service.h.
...
/* + "channel-event drop count is hard-coded zero": despite the hop-specific name
 * (kept as-is -- see mtek_capture_service.h's own doc comment on why this symbol
 * was not renamed), this is this whole session's own general periodic driver,
 * called regardless of whether hop mode is active. */
void mtek_capture_channel_hop_tick(uint64_t now) {
    /* token/boot_epoch were previously read completely UNLOCKED here -- a
     * genuine data race against cap_lock- protected writers
     * (handle_capture_start's own reinit, frame_cb's own mutations,
     * capture_teardown). Folded into the SAME lock acquisition as the rest of
     * this tick's own snapshot, immediately below.
     *
     * session_generation is now captured HERE, as part of this SAME identity
     * snapshot, tied to snap_token -- previously it was re-read fresh from s_cap
     * much later (immediately before the publish guard call), by which point a
     * replacement session could already occupy s_cap, handing the guard call a
     * generation value that belongs to the WRONG session entirely. */
    ...
    /* Acquired BEFORE the final identity re-validation below and held across the
     * ENTIRE window this item's own requirement names -- final immutable-token
     * validation, the real hal-> set_channel HAL action, and the post-HAL
     * state/event commit re-validation. handle_capture_start blocks on this SAME
     * lease before reinitializing s_cap, so a replacement session's own reinit
     * literally cannot happen anywhere inside this window -- not merely
     * detected-after-the-fact by a state-commit re-check, but structurally
     * excluded for the whole HAL action too. Released before this tick's own
     * emit_event call below (never held across that external call) and on every
     * early-return path. */
    cap_action_lock();

    /* Every gate below re-checks s_cap.token == snap_token -- the identity THIS
     * invocation captured at entry -- in addition to hop_active. Checking
     * hop_active alone cannot distinguish "my own session is still hopping" from
     * "a completely different, newer session that also happens to be hop-mode
     * now occupies s_cap" (handle_capture_start's own memset-and-reinit on every
     * new session reuses the same static storage); checking hop_active alone
     * also cannot detect a plain STOP with no replacement, which
     * capture_teardown now clears explicitly (item 1) precisely so this check
    ...
    /* Test-only: lets a deterministic test pause exactly here -- the FINAL
     * identity validation above has just passed, this lease is held, and nothing
     * below has acted on it yet. This is exactly the point a real target's
     * scheduler could preempt this tick for an arbitrarily long time; while
     * paused, a concurrent STOP can still complete (it needs no lease of its
     * own) but a concurrent CAPTURE_START attempting to replace genuinely BLOCKS
     * on cap_action_lock until this tick resumes and releases it below -- see
     * mtek_capture_ service.h's own doc comment. */
    if (s_hop_tick_pause_hook) s_hop_tick_pause_hook();

    /* The real radio channel switch below is a HAL round-trip that must never be
     * held under the publish guard (requirement 5: the guard covers only final
     * validation, shared-state publication, and queue emission) -- it also
     * happens regardless of whether later turns out to be stale, matching
     * mtek_wifi_restore_and_release's own "the radio state must be dealt with
     * honestly regardless" precedent elsewhere in this tree. cap_action_lock
     * (held since before the final validation above) is what actually excludes a
     * replacement session's own reinit for the whole duration of this call, not
     * merely the identity check immediately before it. */
    const mtk_wifi_hal_t *hal = mtek_wifi_get_hal();
    ...
    /* The real running dropped-frame total at the moment of this hop, not a
     * placeholder. */
...
/* See mtek_capture_service.h's own doc comment on this function's declaration
 * for the full rationale. capture_teardown itself already gates on
 * mtk_op_claim_finalization, so this is a safe no-op if the active operation
 * somehow already finalized between the arbiter check and this call. */
mtk_op_id_t mtek_capture_cancel_active_for_peer_reset(void) {
    mtk_op_id_t id = {0, 0};
    /* Coherent-reader fix (diagnosed alongside the admission-guard fix): class
     * and token read from ONE lock acquisition -- two separate
     * mtk_arbiter_active_class/mtk_arbiter_active_token calls could observe a
     * class from one ownership moment and a token from a later one if a
     * concurrent worker changed ownership in between. */
...
/* `rec` is gated on mtk_op_transition's own return value (the linearization
 * point), not a separate, racy pre-check -- matching mtek_wifi_logic.c's own
 * deauth/handshake fix, for the same reason (defensive: capture has no second
 * natural-completion path today, but this closes the same class of bug
 * structurally rather than relying on that remaining true). */
static void capture_teardown(uint32_t token, uint32_t epoch, uint8_t reason, uint8_t status) {
    if (!mtk_op_claim_finalization(token, epoch)) return;
    /* Sink/context/counters copied out under the lock HERE, BEFORE releasing
     * MTK_ARB_M below (mtek_wifi_restore_and_release) -- not after, as the
     * previous ordering did.
     *
     * The earlier ordering's own doc comment already warned "a concurrent
     * handle_capture_start could already have reset s_cap for a brand-new
     * session... by the time an unlocked read here would have run" -- but the
     * locked read still happened AFTER the arbiter release that actually permits
     * a new session to start, so once that release genuinely lets a brand-new
     * handle_capture_start race in and reinitialize s_cap, THIS call's own
     * "locked" read could still pick up the NEW session's own ctx/ sink/counters
     * while reporting THIS (old) token -- the exact "old callback must never
     * read/modify a newly initialized session's state" hazard closes.
     * session_generation is the ORIGINATING session's own, stored once at
     * handle_capture_start time -- never a caller's own ctx, which this function
     * does not always have (mtek_capture_channel_hop_tick's own duration-elapsed
     * call site has none). */
    cap_lock();
    uint32_t total_frames = s_cap.total_frames, dropped_frames = s_cap.dropped_frames, truncated_frames = s_cap.truncated_frames;
    mtk_sink_t sink = s_cap.ctx.sink;
    uint32_t session_generation = s_cap.session_generation;
    /* Explicitly cleared here, under the SAME field-level lock a hop tick's own
     * identity re-validation reads under -- a tick that revalidates `s_cap.token
     * == snap_token` for THIS exact token, strictly after this STOP has already
     * run, must still see its session as no longer legitimately hopping (not
     * only a token mismatch, which only detects a REPLACEMENT, never a plain
     * stop with no replacement yet). This does not by itself need
     * mtek_capture_set_action_lock's own lease -- teardown never touches
     * hop_current_channel or calls set_channel, so it cannot race the tick's own
     * HAL action; only a REPLACEMENT session's own reinit (handle_capture_start)
     * needs to coordinate with that lease. */
    s_cap.hop_active = 0;
    cap_unlock();
    const mtk_wifi_hal_t *hal = mtek_wifi_get_hal();
    if (hal && hal->promisc_stop) hal->promisc_stop();
    /* restore_sta_mode now reports success/failure -- a failure here means the
     * radio's own state is not confirmed safe, even if the capture's own reason
     * for stopping was a clean user request; downgrade the reported status
     * rather than claim a clean stop the HAL itself could not confirm. */
...
static void handle_capture_stop(mtk_request_ctx_t *ctx, const mtk_opcode_entry_t *op,
    ...
    /* Family gate (CAPTURE_START) -- a non-capture token is rejected NOT_FOUND
     * before capture_teardown could finalize a foreign operation or release its
     * arbiter class. */
...
static void handle_capture_poll_read(mtk_request_ctx_t *ctx, const mtk_opcode_entry_t *op,
    ...
    /* Family gate (CAPTURE_START). The s_cap.token match below already rejects
     * any token this service did not mint, but this makes the family check
     * explicit and uniform with every other token- addressed handler -- a
     * foreign token is rejected NOT_FOUND before any s_cap interaction. During
     * an active (mode==1) capture the operation record is RUNNING (non-terminal,
     * never evicted), so a legitimate poll always passes. */
    ...
    cap_lock();
    /* Token/mode were previously read completely UNLOCKED here -- a genuine data
     * race against cap_lock- protected writers (handle_capture_start's own
     * reinit, frame_cb's own ring writes, capture_teardown). Folded into the
     * SAME lock acquisition as the ring read below. */
    ...
    /* Encoded while still holding the lock -- pure computation, no external
     * call, avoiding a second ~1KB capture_record_t stack copy
     * (docs/RESOURCE_BUDGET.md's own heightened scrutiny on this exact class of
     * stack cost). */
...
static void handle_get_transport_counters(mtk_request_ctx_t *ctx) {
    /* Real, live counters (mtk_transport_counters_get, mtek_core.h) instead of a
     * hard-zeroed stub -- see its own doc comment for who increments each field. */
