...
/* The router's own async execution pool (mtek_router.c's
 * MTK_ROUTER_ASYNC_POOL_SIZE) is exactly 4, matching the core contract's
 * declared 4-in-flight-request budget -- a 5th concurrent ACCEPTED_ASYNC
 * dispatch already gets an immediate synchronous NO_MEMORY from the router
 * itself, before this dispatch layer ever needs to remember it. So this
 * transport-level pending table only ever needs to hold at most 4 entries at
 * once; the bug was collapsing that to exactly 1 (a second deferred request
 * would silently overwrite the first's identity, permanently orphaning its
 * eventual response). */
 ...
 * identity and completed response of the latest eight side-effecting requests. A
 * valid duplicate is answered without re-execution."; : "Duplicate-response
 * cache depth | 8 side-effecting requests"). A "side-effecting" request is one
 * whose opcode table entry has `idempotent == 0` -- re-executing it would have
 * an observably different effect than a single execution (e.g. DEAUTH_START
 * restarting its own send cadence), unlike a genuinely idempotent query
 * (GET_VERSION, PING) that this cache does not need to bother with. FIFO ring of
 * the eight MOST RECENTLY COMPLETED such requests' identity + final response, so
 * a retry whose original response never reached the peer (lost on the wire) is
 * answered from here instead of being re-executed -- see
 * dispatch_complete_message's own doc comment for the full three-stage lookup
 * (still-active pending[] slot, then this cache, then a genuinely new dispatch). */
...
/* The SYNCHRONOUS-lifecycle dispatch capture (large enough,
 * MTK_SPI_NATIVE_MAX_MESSAGE, to carry any response up to the reassembly
 * ceiling) is a named field of the dispatch context, not a stack-local inside
 * dispatch_complete_message -- dctx itself is static/ heap-owned by every real
 * caller (main/mtek_spi_runtime.c's spi_runtime_task, which never returns for
 * the whole boot session; every host test allocates its own dctx on that test
 * function's stack, which is fine since host stacks are orders of magnitude
 * larger and this is not a target-constrained context). Safe as a single shared
 * field: SYNCHRONOUS opcodes are dispatched directly on the physical transaction
 * loop's own single thread of control, one at a time, never concurrently with
 * themselves. */
typedef struct {
    uint8_t status;
...
typedef struct {
    /* This field tracks ONLY the currently-recognized PEER (STM32) boot epoch --
     * adopted from an inbound HELLO's own header field (SPI_PROTOCOL_V1.md:
     * "boot_epoch: random nonzero value regenerated on every SENDER boot"), used
     * solely to (a) detect a peer reboot (an inbound HELLO whose boot_epoch
     * differs from this) and (b) reject a CREDIT/CANCEL cell that names a stale
     * peer session. It must NEVER be used to populate the canonical request
     * context's boot_epoch field -- one core-owned value per ESP boot, stamped
     * into every operation token at mint time) nor to stamp any ESP-originated
     * outbound wire cell (HELLO_ACK/RESPONSE/EVENT/STREAM/LINK_ERROR/IDLE all
     * carry the ESP's OWN epoch as "sender", per the same protocol line -- never
     * an echo of whatever epoch the peer last reported). Before this fix, a
     * single field served both roles: adopting the peer's own random epoch here
     * on HELLO, then using that SAME (now peer-owned) value both for
     * ctx.boot_epoch and for outbound wire stamping -- harmless only because
     * every host test happens to initialize the peer's HELLO epoch, this dctx's
     * own seed, and mtk_core's own epoch to the identical value (0x1234 or
     * MTK_TEST_BOOT_EPOCH), which a real STM32 peer (an independent device with
     * its own random RNG) never would. On real hardware this made every
     * operation-token lookup after the first real HELLO fail with NOT_FOUND
     * (START stamps a new record with mtk_core_boot_epoch, but STATUS/STOP
     * dispatched ctx.boot_epoch = the peer's own epoch). */
...
/* #2 "The exact target stack/RAM/heap budget is published and mechanically
 * checked in CI/build scripts": this dctx is the single largest static
 * allocation in the whole firmware (dominated by
 * inbound.data/outbound.data/sync_capture.body, three independent 65,536-byte
 * arrays -- see docs/RESOURCE_BUDGET.md's own "RC6 measured SRAM conflict"
 * section for the full accounting and the contract exception it documents: the
 * accepted 4-independent-reassembly-context guarantee does not fit this chip's
 * published 512KB total SRAM, so this dispatcher keeps exactly ONE full-size
 * inbound context, serialized rather than duplicated -- MTK_SPI_REASM_BUSY,
 * mtek_spi_native_frame.h). This bound is deliberately generous (fails loud at
 * link/compile time long before any real budget ceiling) -- its only job is to
 * catch a future change that silently multiplies this struct's size (e.g.
 * bumping MTK_SPI_NATIVE_MAX_MESSAGE, or adding a second full-size buffer)
 * before it ever reaches a build, not to assert this size is itself safe against
 * the chip's real free-heap budget (which needs hardware measurement -- see
