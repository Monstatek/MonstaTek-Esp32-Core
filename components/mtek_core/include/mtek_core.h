/* Clean-room implementation from MonstaTek contract. Transport-neutral canonical
 * request context, response/event sink, and operation-token lifecycle every
 * adapter (factory UART, native SPI v1, Mtek Compatibility/C3 SPI) constructs a
 * request into and every service dispatches against -- independent of which
 * adapter originated the request. */
#pragma once
#include <stdint.h>
#include <stddef.h>
...
typedef struct mtk_request_ctx {
    ...
    mtk_sink_t sink;
    /* 0 means "not session-scoped" (every adapter except native SPI -- Mtek
     * Compatibility/C3 and factory UART have no peer-reboot concept of their
     * own, matching mtk_core.h's own established boot_epoch doc comment) --
     * never fenced. Native SPI stamps this with mtk_core_ session_generation at
     * the moment it actually dispatches a request (mtek_spi_native_dispatch.c),
     * NOT a wire field and NOT the same concept as boot_epoch: boot_epoch
     * identifies the CURRENT ESP boot session for operation-token scoping (§3.4
     * below); this field identifies WHICH peer session originated this specific
     * request, so a request queued for deferred (async-pool) dispatch before a
     * peer reboot, whose worker has not yet actually started running its handler
     * by the time the reboot is detected, can be recognized as stale and refused
     * (mtk_router.c's own async_trampoline checks this immediately before
     * invoking the handler) rather than minting a brand-new operation
     * indistinguishable from a legitimate request from the NEW peer session. */
