/* Clean-room implementation from MonstaTek contract (002-ble-gatt-service.md,
 * 002-resource-arbiter.md). Portable: no ESP-IDF/NimBLE dependency,
 * host-testable against mtek_ble_hal_t. */
...
/* RC12 blocker round, item 1: (service_id, originating START opcode) per
 * BLE token family (service 0x0002), for the mtk_op_*_family core APIs --
 * a token from another family handed to a BLE STOP/STATUS is rejected
 * NOT_FOUND with no side effect. (GATT handlers, service 0x0003, are
 * separately family-isolated by their own s_gatt.connection_token identity
 * match -- see their doc comments -- because a live GATT connection
 * outlives the GATT_CONNECT operation record it was minted under, so an
 * op-table family check would wrongly reject a valid read after that
 * record is GC'd.) */
 ...
 * effects"): a SEPARATE, dedicated GATT-operation lease/ownership lock --
 * genuinely distinct from s_ble_lock/s_ble_unlock's own field-level mutex
 * above -- see mtek_ble_service.h's own doc comment on mtek_ble_service_
 * set_gatt_op_lease_lock for the full rationale. Every handler that issues
 * a blocking GATT HAL call (discover/discover_chars/discover_descs/read/
 ...
 * acquires the SAME lease before touching s_gatt, so a replacement
 * connection's reinit cannot happen while any prior operation for the
 * connection it would replace is still in flight -- closing the
 * vendor-handle-reuse window for the PHYSICAL HAL action itself, not only
 * for local state. Never acquired by mtek_ble_gatt_tick's remote-
 * disconnect/notify-poll paths or by handle_gatt_disconnect (a local
 * disconnect) -- both remain lock-free with respect to this lease so
 * neither can ever block on (or deadlock against) an in-flight operation;
 * see capture_teardown's own identical, deliberate non-participation
 * (mtek_capture_logic.c, RC11 round 10 item 1) for the same rationale. */
...
/* P0 correction (this round, item 4): see mtek_ble_service.h's own doc
 * comment on mtek_ble_service_mark_tick_task_failed. Ready by default;
...
static void handle_ble_scan_start(mtk_request_ctx_t *ctx, const mtk_opcode_entry_t *op,
     ...
     * actually WINNING finalization for this exact token. s_hal->scan()
     * above can block for the whole requested duration with no cancel
     * hook of its own (unlike AP/STA scan's ap_scan_cancel/sta_scan_
     * cancel) -- if a concurrent peer-session invalidation (or any other
     * STOP-equivalent) already claimed/evicted this token while the call
     ...
     * land in the window between winning and actually publishing below.
     * A single point-in-time re-check cannot close that window either
     * (it only proves the generation had not YET changed the instant it
     * ran). mtk_op_begin_publish_guard, held across the WHOLE publish
     * (both the per-device events and the final summary event, merged
     * into one guarded region below so nothing externally visible ever
     * happens outside it), makes "check" and "the generation actually
     * changing" mutually exclusive instead -- see its own doc comment in
     * mtek_core.h for the full rationale. The already-won op-table
     * transition and the arbiter release below are unaffected by
     * staleness and run OUTSIDE the guard, as before. */
     ...
     * claim (see mtek_ble_cancel_active_for_peer_reset's own BS branch,
     * which deliberately never touches the arbiter itself for this
     * reason) while this call's blocking scan() is still genuinely
     * running; releasing only inside the `won` block would then be
     * skipped entirely and permanently orphan MTK_ARB_BS. The ownership
     * check and the release now happen atomically, in ONE arbiter lock
     * acquisition (mtk_arbiter_release_if_owner) -- this is the ONE
     * place that ever releases it for an in-progress scan, exactly once,
     * whenever the real HAL call actually returns. */
...
static void handle_ble_scan_stop(mtk_request_ctx_t *ctx, const mtk_opcode_entry_t *op,
     ...
     * reuse the slot for a completely different token before this call
     * reads/writes through it. mtk_op_snapshot/mtk_op_transition_by_token
     * both re-validate (token, boot_epoch) under their own lock acquisition
     * every time, so they are immune to that reuse regardless of what now
     * occupies the slot. */
     ...
     * operation is still running"): scan() has no cancel hook of its
     * own -- winning this transition (which happens whenever the
     * worker's own s_hal->scan() call has not yet reached its own
     * mtk_op_claim_finalization) previously released MTK_ARB_BS
     * immediately, letting a brand-new BLE_SCAN_START acquire it and
     * start a second, overlapping scan() call while the first was still
     * genuinely blocked. Only the TOKEN is fenced here now, exactly
     * mirroring mtek_ble_cancel_active_for_peer_reset's own BS handling;
     * handle_ble_scan_start's own tail (now gated on arbiter TOKEN
     * ownership, independent of winning this same transition) is the
     * ONLY place that safely releases this class, once its real HAL
     * call returns. */
...
static void handle_ble_adv_stop(mtk_request_ctx_t *ctx, const mtk_opcode_entry_t *op,
    ...
    mtk_operation_record_t snap;
    /* RC12 item 1: family gate (BLE_ADV_START) -- and the transition itself
     * is family-gated too, so a non-BLE_ADV token can never reach the
     * adv_stop HAL call, the MTK_ARB_BA release, or the BLE_ADV_STOPPED
     * emission for a foreign operation. */
...
static void handle_ble_adv_status(mtk_request_ctx_t *ctx, const mtk_opcode_entry_t *op,
    ...
    /* Frozen shipped List A parity (RC5 independent audit P0): advertising
     * is always non-connectable with no scan response -- see
     * mtek_ble_hal_esp32.c's esp32_ble_adv_start, the real HAL call this
...
/* ---- Signal meter --------------------------------------------------------- */
/* s_sig.ctx (and s_gatt.ctx below) store a byte-for-byte VALUE COPY of
 * *ctx, not a pointer -- safe regardless of the dispatching call's own
 * stack/pool-slot lifetime, since mtk_request_ctx_t holds no pointer back
 * into itself; the only field whose target must independently outlive
 * this session is sink.user, which the dispatching adapter is responsible
 * for pointing at storage stable for the operation's whole lifetime (see
 * mtek_wifi_logic.c's handshake_session_t doc comment for the fuller
 * rationale -- this is the same pattern, applied here from the start
 * rather than fixed after RC5's independent audit found the raw-pointer
 * variant of this bug in the handshake session). */

/* RC5 independent audit P1 "BLE/GATT implementation is not yet parity-
 * complete": "Signal meter scans for 300 ms every 500 ms and declares
 * LOST after one miss, conflicting with the approximately five-second
 * shipping behavior." MTK_SIGNAL_METER_SAMPLE_INTERVAL_MS matches the
 * audit's own stated confirmed figure exactly.
 *
 * RC7 independent audit item 9 "signal LOST now requires three
 * five-second misses (roughly 15 seconds), contradicting shipped
 * approximately-five-second loss behavior": RC6's own
 * MTK_SIGNAL_METER_MISS_TOLERANCE=3 (a disclosed engineering choice --
 * the RC5 audit confirmed one miss was too aggressive but did not itself
 * state an exact tolerance) over-corrected: once the sample interval
 * itself is a real 5 seconds, waiting for 3 CONSECUTIVE misses before
 * declaring LOST takes ~15 seconds total, actively contradicting the
 * confirmed "approximately five-second signal loss" shipped behavior
 * this whole change exists to match. Reverted to 1: the very first
 * missed sample (itself ~5 seconds after the last successful one, given
 * the real sampling cadence above) now declares LOST, matching the
 ...
 * signal_meter_start time. mtek_ble_signal_meter_tick -- an
 * independently-scheduled periodic tick with no request ctx of its own
 * -- validates against THIS stored value via mtk_op_begin_publish_guard,
 * never a caller's own ctx. Every field below is now protected by
 * ble_lock/ble_unlock (requirement 4: this struct had no dedicated lock
...
static void handle_signal_meter_start(mtk_request_ctx_t *ctx, const mtk_opcode_entry_t *op,
    ...
    if (mtk_decode(op->req_desc, &req, req_bytes, req_len, NULL) != MTK_CODEC_OK) { respond_empty(ctx, MTK_STATUS_PROTOCOL_ERROR); return; }
    /* P0 correction (this round, item 4): every sample after the first is
     * only ever delivered by mtek_ble_signal_meter_tick, itself only ever
...
void mtek_ble_signal_meter_tick(void) {
     ...
     * start/_stop's own concurrent mutation on a real target (this tick
     * runs on its own independently-scheduled task, main/app_main.c's
     * ble_tick_task). Snapshotted here, then s_hal->signal_sample (a real
     * HAL round-trip) runs UNLOCKED and un-guarded -- never hold a lock
     * or the publish guard across an external call. */
    ble_lock();
     ...
     * is already immune to slot reuse -- the snapshot re-validates
     * (token, boot_epoch) fresh, under lock, right now. */
    mtk_operation_record_t snap;
    if (!mtk_op_snapshot(token, boot_epoch, &snap) || mtk_op_state_is_terminal(snap.state)) return;

    ble_lock();
    /* P0 correction (requirement 4 "an old callback must never modify a
     * newly initialized session's state"): re-validate s_sig is STILL
    ...
    if (rc != 0) {
         ...
         * remains data-racy": gated on mtk_op_transition's own return
         * value (linearization), not a separate racy pre-check -- a
         * concurrent STOP could otherwise race this exact LOST
...
static void handle_signal_meter_stop(mtk_request_ctx_t *ctx, const mtk_opcode_entry_t *op,
    ...
    if (mtk_decode(op->req_desc, &req, req_bytes, req_len, NULL) != MTK_CODEC_OK) { respond_empty(ctx, MTK_STATUS_PROTOCOL_ERROR); return; }
    mtk_operation_record_t snap;
    /* RC12 item 1: family gate (SIGNAL_METER_START) -- the transition is
     * family-gated too, so a foreign token never releases MTK_ARB_SM or
     * clears s_sig for an operation it does not name. */
    if (!mtk_op_snapshot_family(req.operation_token, ctx->boot_epoch, BLE_SERVICE_ID, SIGNAL_METER_START_OPCODE, &snap)) { respond_empty(ctx, MTK_STATUS_NOT_FOUND); return; }
 ...
 * synchronous dispatch) and mtek_ble_gatt_tick (a separate,
 * independently-scheduled periodic task on a real target) that both
 * read/write it. session_generation is the ORIGINATING connection's own
 * (mtk_request_ctx_t's own field, 0 for every non-native-SPI adapter --
 * never fenced), stored once at handle_gatt_connect's own successful-
 * connect time; mtek_ble_gatt_tick's own notify-delivery path validates
 * against THIS stored value via mtk_op_begin_publish_guard, never a
 * caller's own ctx (the tick has none). */
static struct {
    uint32_t connection_token; uint16_t vendor_handle; uint8_t connected;
    /* dropped_notification_count is now read live from the HAL, not
     * tracked here -- see handle_gatt_status's own doc comment (RC7
     * independent audit item 9). */
...
/* P0 correction (this round, item 2 "make GATT_SUBSCRIBE and
 * GATT_UNSUBSCRIBE operate on one atomically captured identity ... never
 * retrieve the service range separately from the connection identity"):
 * the prior design (a standalone gatt_find_containing_end_handle, called
 * AFTER gatt_snapshot_identity had already released the lock) captured
 * connection_token/vendor_handle in one lock acquisition and then looked
 * up the CCCD-bounding end_handle in a SEPARATE, later lock acquisition,
 * against whatever s_gatt.svc_ranges[] happened to hold at THAT instant --
 * with no correlation back to the identity already captured. A disconnect
 * (and a real, in-flight reconnect -- s_gatt is memset, including
 * svc_ranges[], by handle_gatt_connect's own successful-connect path)
 * landing in the gap between the two calls could make this function
 * return an end_handle that genuinely belongs to a DIFFERENT, newer
 * connection's own discovered ranges, while the caller still believes it
 * is bounding a CCCD search for the connection it originally validated.
 * Both fields are now captured together, under ONE lock acquisition, so
 * "this connection_token names this vendor_handle" and "this attr_handle
 * falls inside this connection's own discovered range" are proven
 * atomically, as a single fact about one connection instance -- never two
 * facts about possibly-different ones. Returns 0 (identity itself invalid:
 * wrong or no connection) or 1 with *out_end_handle set to 0 (identity
 * valid, but attr_handle was never discovered on THIS connection -- the
 * caller must fail PROTOCOL_ERROR, never guess a window) or a real,
 * nonzero end_handle. */
 ...
 * call previously read `s_gatt.vendor_handle` in its OWN separate,
 * unlocked statement, well after gatt_check_conn's own (already-released)
 * lock had confirmed connection_token matched -- a genuine TOCTOU: a
 * disconnect-then-reconnect landing in that window (the BLE stack is free
 * to reuse a small vendor_handle integer for a brand-new, unrelated
 * connection) could hand a HAL call the WRONG physical connection's own
 * canonical vendor_handle even though connection_token still nominally
 * matched at the instant of the earlier check. Snapshots connection_token
 * (the caller's own, already known) together with vendor_handle and
 * session_generation in ONE lock acquisition -- the only atomic proof
 * "this connection_token's own connection is what vendor_handle names,
 * right now". */
...
static void handle_gatt_connect(mtk_request_ctx_t *ctx, const mtk_opcode_entry_t *op,
     ...
     * timeout with no cancel hook of its own (mtek_ble_hal.h has none);
     * everything externally visible below is gated on actually WINNING
     * this transition -- if a concurrent peer-session invalidation
     * already claimed/evicted this token while the call was blocked,
     * this path must never publish s_gatt as connected, emit an event,
     * or release an arbiter class that may by now belong to a
     * completely different, newer GATT_CONNECT. */
     ...
     * state") + "final P0 concurrency-closure round", issue 1: both
     * arbiter release call sites below now go through mtk_arbiter_
     * release_if_owner -- the ownership check and the release happen
     * atomically, in ONE lock acquisition, instead of a separate `mtk_
     * arbiter_active_token() == id.token` check followed by a separate
     * `mtk_arbiter_release` call. In ordinary operation this worker still
     * legitimately holds the class whenever it reaches either site
     * (nothing else ever releases MTK_ARB_GC out from under a still-live
     * token -- the peer-reset canceller's own mid-connect branch
     * deliberately never releases it either, see mtek_ble_cancel_active_
     * for_peer_reset), so this changes no normal-path behavior; it is
     ...
     * reset can still land in the window between winning and actually
     * publishing/emitting below. A single point-in-time re-check cannot
     * close that window either (it only proves the generation had not
     * YET changed the instant it ran). mtk_op_begin_publish_guard, held
     * across the whole publish, makes "check" and "the generation
     * actually changing" mutually exclusive instead -- see its own doc
     * comment in mtek_core.h. `won` still stands regardless (the
    ...
    if (guard_open) {
        ...
        if (connected_now) {
             ...
             * comment above). session_generation is stored here, once,
             * for mtek_ble_gatt_tick's own later notify-delivery guard to
         ...
         * could publish (e.g. a peer-session reset while gatt_connect()
         * above was blocked, or landing in the narrow post-win window):
         * the arbiter lease this operation held is no longer ours to
         * keep, and must never be left orphaned. If the HAL itself
         * genuinely connected in the meantime, that real connection is
         * now untracked by the canonical layer and could never be
         * disconnected again otherwise -- tear it back down immediately
         * rather than leave it silently attached. gatt_disconnect(vh)
         * uses THIS worker's own local vendor handle from its own
         * gatt_connect() call, never shared state, so it can never
         * disconnect a different, newer connection. */
...
static void handle_gatt_disconnect(mtk_request_ctx_t *ctx, const mtk_opcode_entry_t *op,
    ...
    if (do_disconnect) {
         ...
         * depth reason as handle_gatt_connect's own two release sites. */
...
static void handle_gatt_status(mtk_request_ctx_t *ctx, const mtk_opcode_entry_t *op,
     ...
     * silent and never increments the public dropped counter": read live
     * from the HAL (the party that actually owns the bounded notify
     * queue and observes an overflow), not a service-layer field nothing
     * ever incremented. NULL-safe for a HAL that doesn't model this. */
...
static void handle_gatt_discover(mtk_request_ctx_t *ctx, const mtk_opcode_entry_t *op,
    ...
    if (mtk_decode(op->req_desc, &req, req_bytes, req_len, NULL) != MTK_CODEC_OK) { respond_empty(ctx, MTK_STATUS_PROTOCOL_ERROR); return; }
    /* P0 correction (RC11 round 10, item 2): acquired BEFORE the identity
     * snapshot below and held across the whole HAL call + post-HAL
     * revalidation -- see mtek_ble_service.h's own doc comment on
     * mtek_ble_service_set_gatt_op_lease_lock. Blocks a concurrent
     * GATT_CONNECT's own successful-connect reinit from reusing this
     * exact vendor_handle for a replacement connection while this call's
     * own HAL round-trip is still in flight. */
    gatt_op_lease_lock();
    /* P0 correction (Round 8, item 2): the vendor_handle used for the HAL
     * call below is snapshotted together with the connection_token check,
     * in one lock acquisition -- never read separately/unlocked, which
     * could straddle a disconnect-then-reconnect that reuses this exact
     * small integer handle for a different peer. */
     ...
     * zeroed before any HAL call populates it -- defense in depth on top
     * of the HAL's own fix (mtek_ble_hal_esp32.c's disc_svc_cb), so a
     * differently-implemented HAL (a future adapter, or a host-test fake)
     * that does not itself fully populate every field can never leak
     * uninitialized stack memory onto the wire through this array. */
    ...
    /* P0 correction (this round, item 5 "BLE allocation-failure
     * semantics"): a negative return is this HAL's own genuine, distinct
     * failure signal (see mtek_ble_hal_esp32.c's esp32_gatt_discover doc
     * comment) -- clamping it to 0 here silently turned a real resource/
     * IO failure into an apparently successful, genuinely-empty discovery
     * indistinguishable from a peer that truly has zero GATT services.
     * `hal_failed` is checked after the identity re-validation below
     * (matching this file's own established "identity mismatch always
     * takes priority over the HAL's own result" ordering, e.g. handle_
     * gatt_read/handle_gatt_write), never before it. */
    int hal_failed = (n < 0);
    if (n < 0) n = 0;
    if ((unsigned)n > GATT_MAX_SVCS) n = GATT_MAX_SVCS;
    /* P0 correction (Round 8, item 2): re-validate the FULL connection
     * identity (never vendor_handle alone) in the SAME lock acquisition as
     * the svc_ranges[] merge below -- gatt_discover() above can block, and
     * a disconnect (or disconnect-then-reconnect reusing this exact
     * vendor_handle) landing while it was in flight must never let this
     * call's own results merge into a DIFFERENT connection's state. */
    ble_lock();
    ...
    for (; i < (uint16_t)n && cnt < max_items; i++, cnt++) {
        ...
        /* RC8 independent audit P0-5 "Stop GATT UUID stack-data leakage":
         * a 16-bit UUID's own meaningful bytes are only value[0..1] --
         * force the rest to a defined 0 here, at the canonical layer,
         * rather than trusting every current and future HAL
         * implementation to have zeroed its own tail correctly (the real
         * bug this closes: mtek_ble_hal_esp32.c's disc_svc_cb once left
         * these bytes as whatever uninitialized/stale memory preceded
         * them). */
 ...
 * service/char/descriptor counts/listing": new, purely additive opcodes
 * (GATT_DISCOVER_CHARS/GATT_DISCOVER_DESCS) -- GATT_DISCOVER above only
 * ever enumerated services. `start_handle`/`end_handle` must come from a
 * real prior discovery result (a service's own range for
 * GATT_DISCOVER_CHARS, or a characteristic's val_handle+1..containing-
 * service's end_handle for GATT_DISCOVER_DESCS -- the same bounded-
 * range contract gatt_subscribe's own CCCD search already uses); this
 * handler never widens or guesses a range beyond what the caller
 * supplies. */
static void handle_gatt_discover_chars(mtk_request_ctx_t *ctx, const mtk_opcode_entry_t *op,
    ...
    if (mtk_decode(op->req_desc, &req, req_bytes, req_len, NULL) != MTK_CODEC_OK) { respond_empty(ctx, MTK_STATUS_PROTOCOL_ERROR); return; }
    /* P0 correction (RC11 round 10, item 2): see handle_gatt_discover's
     * own doc comment above. */
    gatt_op_lease_lock();
    /* P0 correction (Round 8, item 2): see gatt_snapshot_identity's own
     * doc comment -- vendor_handle for the HAL call below must never be
     * read separately/unlocked from the connection_token check. */
    uint16_t vendor_handle;
    if (!gatt_snapshot_identity(req.connection_token, &vendor_handle, NULL)) { gatt_op_lease_unlock(); respond_empty(ctx, MTK_STATUS_NOT_FOUND); return; }
    ...
    int n = (s_hal && s_hal->gatt_discover_chars)
                ...
                : 0;
    /* P0 correction (this round, item 5): see handle_gatt_discover's own
     * doc comment -- a negative return is a genuine, distinct HAL failure,
     * never silently clamped into an apparently successful empty result. */
    int hal_failed = (n < 0);
    if (n < 0) n = 0;
    if ((unsigned)n > GATT_MAX_SVCS) n = GATT_MAX_SVCS;
    /* P0 correction (Round 8, item 2): this call publishes no shared
     * state, but its own RESPONSE content is meaningless (about whatever
     * connection this vendor_handle happened to reach) once the
     * connection this request was actually issued against is gone --
     * revalidate the full identity before trusting/responding with it. */
...
static void handle_gatt_read(mtk_request_ctx_t *ctx, const mtk_opcode_entry_t *op,
    ...
    /* P0 correction (Round 8, item 2): a disconnect (or disconnect-then-
     * reconnect reusing this exact vendor_handle) while gatt_read() above
     * was blocked must never let its result be trusted/returned as if it
     * genuinely came from THIS connection_token's own peer. */
    int still_valid = gatt_revalidate_identity(req.connection_token, vendor_handle);
    gatt_op_lease_unlock();
    if (!still_valid) { respond_empty(ctx, MTK_STATUS_NOT_FOUND); return; }
...
static void handle_gatt_subscribe(mtk_request_ctx_t *ctx, const mtk_opcode_entry_t *op,
    ...
    if (mtk_decode(op->req_desc, &req, req_bytes, req_len, NULL) != MTK_CODEC_OK) { respond_empty(ctx, MTK_STATUS_PROTOCOL_ERROR); return; }
    /* P0 correction (this round, item 4): every notification this
     * subscription could ever produce is only ever delivered by
     * mtek_ble_gatt_tick's own notify-poll branch, itself only ever called
     * from main/app_main.c's own ble_tick_task -- accepting a subscription
     * that can never actually notify would be dishonest; refuse before
     * ever touching the peer's own CCCD. */
    if (!s_tick_task_ready) { respond_empty(ctx, MTK_STATUS_NOT_READY); return; }
    /* P0 correction (RC11 round 10, item 2 "close stale GATT physical side
     * effects"): acquired BEFORE the identity snapshot below and held
     * across the whole HAL call + post-HAL revalidation/commit -- see
     * mtek_ble_service.h's own doc comment on mtek_ble_service_set_gatt_
     * op_lease_lock. */
    gatt_op_lease_lock();
    /* P0 correction (this round, item 2): connection_token, vendor_handle,
     * AND the CCCD-bounding end_handle are now captured together, in ONE
     * lock acquisition -- see gatt_snapshot_identity_with_range's own doc
     * comment for why the service range must never be looked up
     * separately from the identity that produced it. */
    ...
    /* P0 correction (Round 8, item 2 "make subscription-slot selection and
     * commit atomic"): this is only a fast-fail PRE-check, under lock (the
     * original scan ran completely unlocked, racing concurrent GATT_
     * SUBSCRIBE/_UNSUBSCRIBE calls and mtek_ble_gatt_tick) -- preserved so
     * a subs[] table that is already full is rejected WITHOUT ever writing
     * to the peer's own CCCD, exactly as before. The real, atomic slot
     * claim happens after the HAL call below, in the SAME lock
     * acquisition as the post-call identity re-validation: a slot that
     * looks free here could be raced by a concurrent subscribe before the
     * HAL call returns, and picking it here, then blindly writing it
     * later, is exactly the non-atomic "select, then separately commit"
     * pattern that let two concurrent subscribes silently collide. */
    ble_lock();
    int any_free = 0;
    ...
    /* Item 5 "report HAL and resource failures honestly": an absent HAL/
     ...
     * conflated with an ordinary write failure -- MTK_STATUS_NOT_FOUND
     * (the CCCD descriptor itself was not found) vs MTK_STATUS_IO_ERROR
     * (the CCCD write failed for some other real reason). */
    ...
    /* Atomic: re-validate the FULL connection identity AND claim a slot in
     * ONE lock acquisition -- a disconnect (or disconnect-then-reconnect
     * reusing this exact vendor_handle) while gatt_subscribe() above was
     * blocked must never insert an entry into a DIFFERENT connection's
     * subs[] table, and a slot that was free at the pre-check above must
     * be re-verified here, atomically with the write that claims it. */
    ble_lock();
    int still_valid = s_gatt.connected && s_gatt.connection_token == req.connection_token && s_gatt.vendor_handle == vendor_handle;
    int slot = -1;
    if (still_valid) {
...
/* P0 correction (this round, item 2 "for unsubscribe: remove the existing
 * check-then-use sequence ... do not clear the local subscription before
 * the HAL outcome is handled safely ... do not report OK unconditionally
 * when the HAL operation fails ... prevent a stale unsubscribe from
 * touching a replacement connection"): the prior design had FOUR distinct
 * defects. (1) gatt_check_conn (its own separate lock acquisition) validated
 * connection_token, then RELEASED the lock, then a SECOND, later lock
 * acquisition read s_gatt.vendor_handle unconditionally -- a genuine
 * check-then-use TOCTOU, structurally identical to the one gatt_snapshot_
 * identity was introduced to close for discover/read/write/subscribe. (2)
 * the end_handle lookup (gatt_find_containing_end_handle) was a THIRD,
 * separate lock acquisition with no identity correlation of its own -- the
 * same "service range retrieved separately from the connection identity"
 * hazard subscribe had. (3) the local subs[] entry was cleared UNCONDITIONALLY
 * before the HAL call even ran, so a HAL failure left local state claiming
 * "unsubscribed" while the peer's own CCCD may still be armed and
 * notifying. (4) the response was always MTK_STATUS_OK regardless of the
 * HAL call's own (entirely discarded) return value -- a silent false
 * success. Fixed by mirroring handle_gatt_subscribe's own three-phase
 * shape exactly: one atomic snapshot (identity + range together), a
 * locked pre-check for a genuinely active local subscription (never
 * issue a physical CCCD write for a handle that isn't actually subscribed
 * -- preserves the prior no-op-success behavior for that case), the HAL
 * call itself unlocked, then ONE final lock acquisition that re-validates
 * the FULL identity again (never vendor_handle alone) before clearing any
 * local state -- a stale request whose identity changed mid-call (a
 * disconnect, or disconnect-then-reconnect reusing this exact
 * vendor_handle) can therefore never clear a REPLACEMENT connection's own
 * subscription table, and a genuine HAL failure is reported honestly
...
static void handle_gatt_unsubscribe(mtk_request_ctx_t *ctx, const mtk_opcode_entry_t *op,
    ...
    if (mtk_decode(op->req_desc, &req, req_bytes, req_len, NULL) != MTK_CODEC_OK) { respond_empty(ctx, MTK_STATUS_PROTOCOL_ERROR); return; }
    /* P0 correction (RC11 round 10, item 2): see handle_gatt_subscribe's
     * own doc comment above -- acquired BEFORE the identity snapshot and
     * held across the whole HAL call + post-HAL revalidated commit. */
    gatt_op_lease_lock();
    uint16_t vendor_handle, end_handle;
    if (!gatt_snapshot_identity_with_range(req.connection_token, req.handle, &vendor_handle, &end_handle, NULL)) {
        gatt_op_lease_unlock(); respond_empty(ctx, MTK_STATUS_NOT_FOUND); return;
...
/* RC7 independent audit item 9 -- see mtek_ble_service.h's own doc
 * comment for the full rationale. Set by mtek_ble_gatt_tick's remote-
 * disconnect branch below; cleared by whichever caller (the UART
 * adapter's own background poll) consumes it first. */
static uint8_t s_gatt_remote_disconnect_pending;
/* RC8 independent audit P0-6: the real reason captured alongside the
 * pending flag above -- never a hard-coded placeholder. */
static uint8_t s_gatt_remote_disconnect_reason;

uint8_t mtek_ble_gatt_take_remote_disconnect_notice(uint8_t *reason_out) {
    /* P0 correction (requirement 4): this accessor and mtek_ble_gatt_tick's
     * remote-disconnect branch below run on separate tasks on a real
     * target (the UART adapter's own background poll vs. the BLE tick
     * task) -- both statics need the same dedicated lock as s_gatt. */
    ble_lock();
...
void mtek_ble_gatt_tick(void) {
     ...
     * _disconnect/_subscribe/etc.'s own concurrent mutation of the SAME
     * fields. Snapshotted here, then every HAL/sink call below runs
     ...
     * vendor_handle alone cannot distinguish this connection from a
     * DIFFERENT one the HAL/stack later assigns the same small integer
     * handle to after a disconnect-then-reconnect race. */
    ble_lock();
     ...
     * connection/arbiter state stale." Real cleanup -- releases the GC
     * arbiter lease and clears connected-state bookkeeping -- the moment
     * a genuine peer-initiated disconnect is observed, not only the next
     * time some unrelated call happens to fail against the now-dead
     * connection. There is no confirmed wire event for this in the
     * accepted facts package (only the caller-driven GATT_DISCONNECT
     * request/response exists), so this fixes the STATE going stale;
     * a subsequent GATT_STATUS/any GATT_* call now correctly observes
     * the disconnect via s_gatt.connected (checked by every handler's own
     * identity snapshot), exactly as if the caller had issued GATT_
     * DISCONNECT itself.
     * RC7: also sets s_gatt_remote_disconnect_pending so the factory
     * UART adapter can print its own frozen transcript line for this
     * case too (see mtek_ble_service.h's doc comment on the accessor
     * above -- deliberately not routed through emit_event, so this
     * branch needs no publish guard: nothing is ever emitted). */
    uint8_t disc_reason = 0;
    if (s_hal->gatt_poll_disconnected && s_hal->gatt_poll_disconnected(vendor_handle, &disc_reason)) {
        ble_lock();
        /* P0 correction (requirement 4 "an old callback must never
         * modify a newly initialized session's state"; Round 8 item 2:
         * full identity, never vendor_handle alone): re-validate -- s_gatt
         * could have already been disconnected-and-reconnected for a
         * wholly different connection since the snapshot above (a
         * concurrent GATT_DISCONNECT + a brand-new GATT_CONNECT, both
         * genuinely possible while this HAL poll call was in flight),
         * possibly reusing this exact vendor_handle for the new one. A
         * mismatch on EITHER field means this stale disconnect notice no
         * longer applies to whatever connection s_gatt now holds. */
 ...
 * codebase's HAL contract for it is not a genuinely backgroundable
 * operation the way DEAUTH/HANDSHAKE are), so MTK_ARB_BS should never
 * actually be found still active here -- handled defensively anyway,
 * never assumed. GATT_CONNECT's own operation token is already terminal
 * (COMPLETED) by the time a connection exists (002-canonical-core-
 * contract.md §4.2's own STA_DISCONNECT/GATT_DISCONNECT carve-out: not
 * operation-token-addressed for teardown), so only the connection
...
mtk_op_id_t mtek_ble_cancel_active_for_peer_reset(void) {
    ...
    switch (active) {
        case MTK_ARB_BS:
             ...
             * handle_ble_scan_start's own tail (now gated on arbiter
             * TOKEN ownership, independent of winning this same
             * transition) is the ONLY place that safely releases this
             * class, exactly once the real HAL call returns. */
        ...
        case MTK_ARB_SM:
            if (mtk_op_transition_by_token(tok, epoch, MTK_OPS_STOPPED, MTK_STATUS_OK, now_ms())) {
                 ...
                 * a genuine, previously-missed data race against mtek_ble_
                 * signal_meter_tick's own concurrent access, matching
            ...
            break;
        case MTK_ARB_GC:
            /* Two distinct, mutually exclusive cases: (1) still mid-
             * connect (the operation itself is not yet terminal) -- fence
             * it via the SAME token-based transition handle_gatt_
             * connect's own tail uses (a worker that later loses this
             * race tears down any real HAL-level connection it still
             * manages to form, on its own); or (2) already connected
             * (the operation's own token is already terminal/COMPLETED --
             * 002-canonical-core-contract.md's own STA_DISCONNECT/GATT_
             ...
             * still genuinely be running; releasing here would let a
             * brand-new GATT_CONNECT acquire it and start a second,
             * overlapping connect against the same peripheral/link.
             * handle_gatt_connect's own tail (now token-ownership-gated,
             * independent of winning this same transition) is the ONLY
             * place that safely releases the class for an in-progress
             * connect. Case (2) is a genuinely idle, already-established
             * connection with no HAL call in flight -- gatt_disconnect()
             * there completes synchronously, so releasing immediately
             * after it returns is safe and unchanged. */
            if (mtk_op_transition_by_token(tok, epoch, MTK_OPS_STOPPED, MTK_STATUS_OK, now_ms())) {
                id.token = tok; id.boot_epoch = epoch;
            } else {
                /* P0 correction (follow-up read-only audit, "Round 8: final
                 ...
                 * written, completely UNLOCKED here -- a genuine,
                 * previously-missed data race against every other s_gatt
                 * accessor in this file (handle_gatt_connect/_disconnect/
                 * mtek_ble_gatt_tick), all of which already lock. Snapshot
                 * under lock, commit connected=0 under the SAME lock
                 * (matching handle_gatt_disconnect's own established
                 * pattern exactly), then call the HAL and release the
                 * arbiter OUTSIDE it. */
                ble_lock();
