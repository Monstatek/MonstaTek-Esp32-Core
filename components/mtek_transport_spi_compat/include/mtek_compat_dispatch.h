 ...
 * Legacy SPI Compatibility `m1_link` REQUEST frames (mtek_compat_frame.h) into canonical
 ...
 * helper functions that build a Legacy SPI Compatibility-shaped reply from one or more
 * canonical dispatch calls. */
#define COMPAT_CAP_BODY_MAX MTK_COMPAT_MAX_REASSEMBLY_PAYLOAD
typedef struct {
    uint8_t status;
    uint8_t body[COMPAT_CAP_BODY_MAX];
    size_t body_len;
 ...
 * FRAG cells (matching mtk_compat_reassembly_feed's own inbound
 ...
 * chunk. Drained one cell per poll via mtek_compat_dispatch_poll_outbound,
 * matching the Legacy SPI Compatibility transport's own poll-driven, one-outstanding-
 * message model. */
typedef struct {
    uint8_t active;
    uint16_t msg_id;
    uint8_t final_msg_type; /* MTK_COMPAT_MSG_RESP or MTK_COMPAT_MSG_NAK */
    uint16_t total_len;
    uint16_t sent_offset;
    uint8_t data[MTK_COMPAT_MAX_REASSEMBLY_PAYLOAD];
} mtk_compat_outbound_t;
...
typedef struct {
     ...
     * matching *_STOP; each family can have at most one Legacy SPI Compatibility-originated
     ...
     * the wire). Lets a host test prove every one of the 44 compat_spi-
     * SUPPORTED canonical opcodes is reachable through its real,
     * documented Legacy SPI Compatibility wire opcode -- not merely present in a registry. */
     ...
     * mtek_compat_dispatch_request call that kicked it off. Legacy SPI Compatibility is
    ...
    /* CAPTURE_START's confirmed Legacy SPI Compatibility response format (raw 4-byte LE
    ...
    /* RC12 blocker round, item 2 "real deferred compatibility execution":
     * AP_SCAN_START/STA_SCAN_START/GATT_CONNECT produce their real Legacy SPI Compatibility
     ...
     * confirmed Compatibility response (for AP scan, by issuing the canonical
     * AP_SCAN_RESULTS_PAGE and building the same network-list bytes the
     * synchronous path builds). No new Legacy SPI Compatibility wire event or opcode is
     ...
     * Legacy SPI Compatibility dispatch is strictly single-outstanding-request, one
...
/* `resp_payload` must have capacity >= MTK_COMPAT_SINGLE_CELL_PAYLOAD_MAX
 * (one wire cell's worth); a response over that size is staged into
 * dctx->outbound and drained via mtek_compat_dispatch_poll_outbound. */
 ...
 * outbound, emits a well-formed IDLE cell (msg_type=MTK_COMPAT_MSG_IDLE,
