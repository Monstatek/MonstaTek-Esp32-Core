 ...
 * Mtek Compatibility/C3 `m1_link` REQUEST frames (mtek_compat_frame.h) into canonical
 ...
 * matching the Mtek Compatibility transport's own poll-driven, one-outstanding-
 * message model. */
typedef struct {
    uint8_t active;
    uint16_t msg_id;
...
typedef struct {
     ...
     * matching *_STOP; each family can have at most one Mtek Compatibility-originated
     ...
     * the wire). Lets a host test prove every one of the 44 compat_c3-
     * SUPPORTED canonical opcodes is reachable through its real,
     * documented Mtek Compatibility wire opcode -- not merely present in a registry. */
     ...
     * mtek_compat_dispatch_request call that kicked it off. Mtek Compatibility is
    ...
    /* CAPTURE_START's confirmed Mtek Compatibility response format (raw 4-byte LE
