 ...
 * proven safe for native SPI and Legacy SPI Compatibility event queues. */
...
static int mtek_uart_line_is_recognized(const mtk_uart_adapter_state_t *st, const char *line) {
    ...
    if (st->mode == MTK_UART_MODE_WIFI) {
        uint8_t pcap_channel;
        uint32_t pcap_duration_ms;
        if (mtek_uart_pcap_parse_start(line, &pcap_channel,
                                       &pcap_duration_ms)) return 1;
