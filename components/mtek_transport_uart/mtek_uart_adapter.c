...
/* Persistent, queue-backed sink ---- Used only for the three commands whose
 * canonical operation retains its sink across the synchronous call for later
 * background HAL-callback delivery (see mtek_uart_adapter.h's session_queue doc
 * comment). `user` is `&st->session_queue` -- adapter-owned,
 * boot-session-persistent -- never a stack-local object, so it stays valid for
 * as long as the canonical service's own session state (handshake_session_t,
 * s_sig, s_gatt) chooses to keep calling through it, exactly the same pattern
