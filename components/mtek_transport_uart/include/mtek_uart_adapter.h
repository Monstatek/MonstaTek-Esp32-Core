 ...
 * (001-command-behavior-matrix.md). Factory M1 UART compatibility adapter:
 * parses the legacy ASCII REPL grammar into canonical requests and formats
 * canonical responses/events back into the legacy printed lines -- never
 * calling a legacy handler and reparsing its output
 * (002-adapter-translation-matrix.md Sec 1.4). */
#pragma once
#include <stddef.h>
#include <stdint.h>
...
typedef struct mtk_uart_adapter_state {
    ...
    /* Wi-Fi: cached scan tables and target selection (002-wifi-service.md
     * Sec 3: "target selection is not a canonical operation" -- select/
     * list live only here, never as a wire opcode). */
     ...
     * needed to dispatch BLE_DEVICE_DETAILS for `list <id>`/`list -d`
     * (the same result_generation the scan itself produced -- never a
     * guessed/stale one). */
     ...
     * invalid request context": persistent, adapter-owned (this struct
     * lives for the whole REPL task's boot-session lifetime -- see
     * app_main.c's uart_repl_task, which allocates one on its own stack
     * once and loops on it forever) delivery queue for the three UART
     * commands whose canonical operation has a HAL callback that can fire
     * after the synchronous command call has already returned:
     * `handshake` (HANDSHAKE_EVENT), `signal` (SIGNAL_METER_UPDATE/LOST),
     * and `connect` (GATT_VALUE_EVENT notifications/indications). Every
     * other UART command still uses a stack-local capture, which remains
     * safe because the router never defers a FACTORY_UART-profile
     * dispatch (mtek_router.c) and those commands' own canonical services
     * never touch their sink again once the synchronous call returns. */
     ...
     * portable component has no ESP-IDF dependency (host-testable) so it
     * cannot call esp_restart() itself -- it sets this flag instead; the
     * target-specific REPL task (app_main.c) checks it after each
     * mtek_uart_process_line call and performs the real delayed restart,
     * matching the established portable-core/thin-target-glue split used
 ...
 * independently measured by the audit) is static in app_main.c's
 * uart_repl_task, not stack-local -- this bound only guards against a
 * future silent size regression, not a hardware-measured budget claim. */
#include <assert.h>
 ...
 * still knowingly incomplete": formats the exact boot warning + full
 * command reference (identical text to the `help` command) into `out`.
 * The target REPL loop (app_main.c) must call this exactly once, before
 * writing the first `>> ` prompt -- side-effect-free, does not touch
 * `st`. Returns the number of bytes written (excluding the NUL). */
 ...
 * grammar recognition": side-effect-free -- returns 1 if `line` matches
 * a real, currently-dispatchable command grammar (mode-aware, mirroring
 * mtek_uart_process_line's own dispatch table exactly), 0 for noise,
 * partial input, an unknown command, or an empty line. The REPL loop
 * (app_main.c) checks this BEFORE ever attempting mtk_transport_claim_try,
 * so arbitrary/garbage bytes can never permanently lock out a genuine SPI
 * peer for the rest of the boot session. */
