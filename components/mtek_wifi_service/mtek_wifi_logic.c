 ...
 * 002-resource-arbiter.md). Portable: no ESP-IDF dependency, host-testable
 * against mtek_wifi_hal_t (fake HAL for host tests, real ESP-IDF HAL on
 * target). */
 ...
 * the (service_id, originating START opcode) each token-addressed handler
 * passes to the mtk_op_*_family core APIs so a token minted by a DIFFERENT
 * operation family (e.g. a live STA_SCAN token handed to AP_SCAN_STOP) is
 * rejected NOT_FOUND with zero side effect, instead of being found and
 * acted upon. Each STOP/STATUS/READ opcode consumes a token minted by its
 * family's own START. */
...
/* RC8 independent audit P0-3 -- see mtek_wifi_service.h's own doc comment
 * on mtek_wifi_service_set_lock for the full rationale. Guards s_deauth
 * and s_hs (declared with their own usage further down this file). */
 ...
 * unlocked dereference of a RETAINED record pointer, read repeatedly
 * across many loop iterations -- exactly the hazard mtek_core.h's own
 * doc comment on mtk_op_find warns about, "a session that RETAINS a
 * mtk_operation_record_t* across multiple separate calls/ticks/
 * callbacks") and a concurrent STOP's own mtk_op_transition write to the
 * SAME record. mtk_op_transition's own return value already correctly
 * linearizes which caller performs cleanup (a separate, already-fixed
 * concern) -- this closes the OTHER, distinct hazard: the loop's own
 * repeated "should I keep going" check must go through a locked
 * snapshot, not a raw pointer read, even though a stale read here was
 * always logically benign (worst case: one extra loop iteration before
 * noticing termination). Fails safe (treats "not found" as terminal). */
 ...
 * cancellation, waits bounded for the worker's own natural completion,
 * and on a genuine timeout force-finalizes the TOKEN ONLY (never the
 * arbiter/radio, which stays handle_ap_scan_start's/handle_sta_scan_
 * start's own tail's sole responsibility once its real blocking
 * ap_scan()/sta_scan() call actually returns). Kept as ONE implementation
 * so the two callers can never independently drift. */
...
static void handle_ap_scan_start(mtk_request_ctx_t *ctx, const mtk_opcode_entry_t *op,
     ...
     * strict-List-A behavior change, now approved): a negative return is
     * a real transactional-entry failure (callback/channel/promiscuous-
     * mode step inside esp32_ap_scan, e.g. capture_prior_state_once
     * failing, or esp_wifi_set_mode/_start failing) -- previously silently
     * rewritten to 0 and reported as "scan completed, 0 APs found",
     * indistinguishable from a genuine empty result. Same linearization
     * and honest-failure shape as handle_sta_scan_start's own identical
     ...
     * half actually takes effect), independent of whether this worker
     * also wins the op-table claim. A peer-reset canceller may have
     * already forced this exact token to a terminal state after its own
     * bounded quiescence wait timed out (see mtek_wifi_cancel_active_
     * for_peer_reset's WS branch) while this worker's blocking HAL call
     * is still genuinely running; in that case `won` is false here (the
     * canceller already claimed finalization) but the arbiter class is
     * still owned by my_token (the canceller deliberately never touches
     * it), so this call is still the ONE place that safely performs the
     ...
     * across the ENTIRE publish (never merely a single point-in-time
     * re-check, which cannot close the window between itself returning
     * and the actual publish executing -- see mtek_op_begin_publish_
     * guard's own doc comment in mtek_core.h for why). `won` alone only
     * proves nobody else finalized this token BEFORE this exact instant;
     * it says nothing about a peer-session reset landing in the window
     * between winning and actually publishing/emitting -- the guard
     * closes that window to zero width by making it mutually exclusive
     * with mtk_core_bump_session_generation itself. restore/release
     * happens OUTSIDE the guard (a real, potentially slow HAL round-trip
     * that must never block a peer-session reset); only the fast,
     * fully-in-memory state write + event emit are held under it. */
    if (n < 0) {
    ...
    if (won && mtk_op_begin_publish_guard(ctx->session_generation)) {
        /* RC10 independent correction order P0 "STA scan result/session
         * state is unsynchronized" (audit extended to s_ap too): this
         * write can run on a different worker task than a concurrent
         * AP_SCAN_STATUS/RESULTS_PAGE/DETAILS read of the exact same
         * struct -- guard the whole publish with this file's own
         * established wifi_lock/unlock (mtek_wifi_service_set_lock),
         * matching s_deauth/s_hs's own RC8 fix. The results themselves
         * are published here (they are real and correct; only the
         * radio-restoration step, already handled above, was ever in
         * question). */
        wifi_lock();
 ...
 * handle_sta_scan_stop's own identical fix (RC10) exactly, now that
 * esp32_ap_scan_cancel exists (mtek_wifi_hal_esp32.c's own RC11 addition)
 * to make an in-flight esp_wifi_scan_start(..., true) return promptly
 * instead of waiting out its own full duration. STOP no longer transitions
 * or restores/releases itself -- it only SIGNALS cancellation and waits
 * (bounded) for handle_ap_scan_start's own worker to reach a terminal
 * state, which alone ever performs restore/release for this opcode
 * (never both paths). A timeout that is not yet terminal reports BUSY,
 * never a false STOPPED/BUSY-free claim. */
static void handle_ap_scan_stop(mtk_request_ctx_t *ctx, const mtk_opcode_entry_t *op,
     ...
     * have echoed back), so no pointer of any kind needs to survive past
     * this one call. */
    mtk_operation_record_t snap;
    /* RC12 item 1: family gate -- a token minted by any family other than
     * AP_SCAN_START is rejected NOT_FOUND here, before any cancel signal,
     * arbiter, radio, or worker interaction. */
    if (!mtk_op_snapshot_family(req.operation_token, ctx->boot_epoch, WIFI_SERVICE_ID, AP_SCAN_START_OPCODE, &snap)) { respond_empty(ctx, MTK_STATUS_NOT_FOUND); return; }
...
static void handle_sta_scan_start(mtk_request_ctx_t *ctx, const mtk_opcode_entry_t *op,
     ...
     * the transactional radio lifecycle": a negative return is a real
     * transactional-entry failure (callback/channel/promiscuous-mode
     * step) -- previously silently rewritten to 0 and reported as "scan
     * completed, 0 stations found", indistinguishable from a genuine
     * empty result. Same linearization as every other List B operation
     * in this file: only the side that actually wins mtk_op_transition
     * performs cleanup/restore/emits the terminal event (a concurrent
     * STOP -- now genuinely possible mid-scan via s_hal->sta_scan_cancel
     * -- may have already done so). */
    /* RC11 independent correction order P0 "operation finalization is not
     * exactly-once/truthful": both branches below used to call
     * mtk_op_transition directly (locking in a status BEFORE restore's
     * real result was known) and release the arbiter unconditionally
     * regardless of that result -- defeating mtek_wifi_restore_and_
     * release's own "quarantine on failure, never release" contract.
     * mtk_op_claim_finalization (claim) -> mtek_wifi_restore_and_release
     ...
     * comment): restore/release below is called unconditionally (the
     * atomic ownership check now lives inside mtek_wifi_restore_and_
     * release itself), never solely on winning the op-table claim, so a
     * peer-reset canceller's forced timeout finalization can never
     * orphan the WS arbiter class while this worker's blocking HAL call
     * is still genuinely running. State publish/event emission remain
     * strictly gated on `won` AND mtk_op_begin_publish_guard, held
     * across the entire publish -- not a single point-in-time re-check,
     * which cannot close the window between itself returning and the
     * actual publish executing. restore/release always runs OUTSIDE the
     * guard (a real, potentially slow HAL round-trip). */
    if (n < 0) {
     ...
     * failure-atomic" + RC11 "keep the radio lease quarantined, never
     * report/expose it as healthy/free on a restore failure": downgrade
     * only the reported status, and let the shared wrapper decide
     * whether the arbiter lease is actually safe to release. Called
     * unconditionally -- see mtek_wifi_restore_and_release's own doc
     * comment for why no ownership pre-check is needed here any more.
     * Runs BEFORE the publish guard below (never held under the same
     * lock a peer-session reset needs). */
    ...
    if (won && mtk_op_begin_publish_guard(ctx->session_generation)) {
        /* RC10 independent correction order P0 "STA scan result/session
         * state is unsynchronized": s_sta can be read by
         * handle_sta_scan_status/_results_page from a concurrent worker
         * task -- publish the whole snapshot under this file's own
         * established wifi_lock/unlock (mtek_wifi_service_set_lock). The
         * results themselves are published here (real and correct; only
         * the radio-restoration step, already handled above, was ever in
         * question). */
        wifi_lock();
 ...
 * before the worker has stopped touching it": this handler used to
 * signal cancellation and IMMEDIATELY restore/release itself, gated only
 * on winning mtk_op_transition -- but esp32_sta_scan's own blocking call
 * can remain in flight for up to one poll interval (and its own teardown
 * afterward) after the cancel signal is seen, so a second operation could
 * acquire the "released" radio while the old worker was still tearing
 * down. STOP is now a real quiescence handshake instead: it only
 * SIGNALS cancellation, then waits (bounded) for handle_sta_scan_start's
 * own worker to actually reach a terminal state -- that worker remains
 * the ONE place that ever calls restore_sta_mode/releases the arbiter for
 * this opcode (never both paths, matching "do not restore from both STOP
 * and worker paths"). A bounded timeout that is NOT yet terminal fails
 * safe: it reports BUSY (the radio may still be in use) rather than
 * falsely claiming STOPPED/BUSY-free. */
static void handle_sta_scan_stop(mtk_request_ctx_t *ctx, const mtk_opcode_entry_t *op,
     ...
     * through a fresh locked mtk_op_snapshot (op_is_terminal_now's own doc
     * comment, shared with handle_deauth_start's identical rule) -- never
     * a raw `rec->state`/`rec->final_status` dereference retained across
     * this function's own bounded wait, which could otherwise observe a
     * DIFFERENT operation's data if this exact slot were recycled for a
     * new token in the meantime. */
    mtk_operation_record_t snap;
    /* RC12 item 1: family gate (STA_SCAN_START) -- see handle_ap_scan_stop. */
    if (!mtk_op_snapshot_family(req.operation_token, ctx->boot_epoch, WIFI_SERVICE_ID, STA_SCAN_START_OPCODE, &snap)) { respond_empty(ctx, MTK_STATUS_NOT_FOUND); return; }
    uint32_t my_token = req.operation_token, my_epoch = ctx->boot_epoch;
...
/* RC10 independent correction order P0 "STA scan result/session state is
 * unsynchronized" (audit extended to s_sta_connected too): this file's
 * own established wifi_lock/unlock protects the single write/read pair
 * below the same way it now protects s_ap/s_sta. */
...
static void handle_sta_connect(mtk_request_ctx_t *ctx, const mtk_opcode_entry_t *op,
     ...
     * unconditional release here (this call's own connect() above has no
     * cancel hook and can block for up to `timeout`) could otherwise tear
     * down a BRAND-NEW STA_CONNECT's own lease if a peer-session reset
     ...
     * active_token() == id.token` check followed by a separate `mtk_
     * arbiter_release` call -- closing the real (if narrow) window
     * between those two calls where a concurrent reassignment could have
     * been released out from under a newer operation. */
    ...
    /* Gate EVERYTHING else externally visible (s_sta_connected publish,
     * the STA_CONNECT_COMPLETE event) on actually WINNING this
     ...
     * about a peer-session reset landing in the narrow window between
     * winning and actually publishing/emitting below. A single point-in-
     * time re-check cannot close that window either (it only proves the
     * generation had not YET changed the instant it ran -- a reset can
     * still land between the check returning and the state write/event
     * emit below actually executing). mtk_op_begin_publish_guard makes
     * "check" and "the generation changing" genuinely mutually exclusive
     * instead: it is held across the ENTIRE publish (state write AND
     * event emission) and is backed by the SAME lock mtk_core_bump_
     * session_generation itself briefly holds, so a bump cannot complete
     * while this is open. `won` still stands regardless (the transition
     * already happened and must not be undone) -- only publish is gated
     * on the guard. */
...
static void handle_sta_disconnect(mtk_request_ctx_t *ctx) {
     ...
     * is genuinely connected (s_sta_connected true), the class has
     * ALWAYS already been released, regardless of whether the attempt
     * succeeded or failed. So there is never a legitimate reason for
     * this handler to touch the arbiter directly: if WMC is still held
     * when this runs, a connect attempt is genuinely still in flight on
     * another thread (connect() has no cancel hook) and releasing it now
     * would let a brand-new STA_CONNECT start a second, overlapping
     * connect() call; if WMC is already free, there is nothing to
     * release. Disconnect/HAL teardown is likewise gated on genuinely
     * being connected -- calling disconnect() while nothing is connected
     * (or while a connect attempt is still resolving) is not meaningful
     * and no longer attempted. */
    wifi_lock();
...
/* RC5 independent audit P1 "Deauth 'all' and continuous operation are not
 * correctly implemented": live progress for a still-RUNNING operation
 * (DEAUTH_STATUS's own `sent` field used to always report 0, and
 * count=0/"until stopped" completed after a single round-robin pass
 * instead of genuinely running until STOP). Session-scoped (matches the
 * arbiter's single-active-D-class model: at most one deauth operation
 * runs at a time), read by handle_deauth_status below. */
/* RC7 independent audit P0 "Shared operation/session state remains
 * data-racy": stores the TOKEN, not a retained `mtk_operation_record_t *`
 * -- the record's own table slot can be evicted (GC, MTK_OP_RETENTION_MS
 * after this operation went terminal) and later reused by a COMPLETELY
 * different, unrelated operation at the exact same address. Comparing a
 * retained pointer's identity against a freshly-`mtk_op_find`-ed one (as
 * this used to) would then wrongly match that unrelated operation's own
 * valid token to this stale session, reporting THIS session's leftover
 * total_sent as if it were the new operation's live progress -- a
 * classic ABA hazard. A token is effectively unique for the life of the
...
/* RC11 independent correction order P0 "operation finalization is not
 * exactly-once/truthful" (same defect and same fix shape as mtek_capture_
 * service.h's own capture_teardown and this file's own handshake_finish,
 * above): the natural loop-exit path and handle_deauth_stop used to call
 * mtk_op_transition directly, locking in `status` as the record's own
 * final_status BEFORE restore_sta_mode's real result was known (a
 * DEAUTH_STATUS query racing either path could observe a final_status of
 * OK that the DEAUTH_STOPPED event's own separately-computed field
 * simultaneously contradicted), and released the arbiter unconditionally
 * regardless of restore's own result -- defeating mekt_wifi_restore_and_
 * release's own "quarantine on failure, never release" contract for this
 * opcode specifically. Claims the single finalization path
 * (mtk_op_claim_finalization), performs the real cleanup via that shared
 * wrapper, THEN writes the one truthful terminal state/status -- never
 * the other order. Returns 1 (and writes *out_final_status) only if THIS
 * call actually performed finalization; 0 if it lost the race (another
 * path already has, or is already doing so). */
...
static void handle_deauth_start(mtk_request_ctx_t *ctx, const mtk_opcode_entry_t *op,
     ...
     * (e.g. esp_wifi_80211_tx rejecting an unsupported frame type on the
     * real target, see mtek_wifi_hal_esp32.c) is tracked separately and
     * never silently folded into a "sent" count.
     *
     * count=0 ("run until stopped", RC5 independent audit P1 "Deauth
     * 'all' and continuous operation are not correctly implemented")
     * genuinely runs until STOP transitions rec->state to terminal, WHEN
     * this handler is actually executing on a deferred background
     * worker -- true on every real target build, where the router's
     * async runner is registered once, early, before any dispatch can
     * occur (main/mtek_spi_runtime.c), so a genuinely long-running loop
     * here never blocks the transport's own request-dispatch loop, which
     * remains free to receive and dispatch the concurrent STOP that ends
     * it. mtk_router_running_on_worker() (thread-local -- correct even
     * with up to 4 concurrent deferred operations) distinguishes that
     * from running synchronously on the calling thread -- whether
     * because no runner is registered at all (every host test that does
     * not itself register one, matching the deterministic single-
     * threaded model the rest of this suite relies on) or because the
     * dispatching adapter is FACTORY_UART, which mtk_router_dispatch's
     * own transport-aware gate never defers even when a runner is
     * registered (see mtek_router.c and its P0-1 fix): blocking
     * indefinitely in either case would hang the caller forever with no
     * way for any STOP to ever reach it, so both intentionally fall back
     * to one bounded round-robin pass -- an honest reflection of "this
     * call has no mechanism available to run in the background", not a
     * silent behavior gap. A bounded per-round pacing delay
     * (s_hal->pace_delay_ms, NULL-safe) keeps a genuinely continuous
     * operation from spinning a CPU core and gives the terminal-state
     * check below a real, frequent chance to observe a concurrent STOP
     * promptly, rather than only between whole target-set passes. The
     * per-target-per-round transmit cadence itself is a disclosed
     * engineering choice (not an independently confirmed timing value
     * from the accepted facts package): one full target-set round, then
     * a short pace delay, repeated. */
     ...
     * token/boot_epoch pair (id, copied out atomically at mint time -- see
     * mtk_op_alloc_id's own doc comment) is immutable for the operation's
     * whole lifetime and is then used for every "should I keep going"
     * check below via a locked snapshot (op_is_terminal_now), never a raw
     * pointer/state dereference retained across this loop's many
     * iterations. */
    /* my_epoch is id.boot_epoch (the mtk_core-internal epoch this
     * record was actually allocated under, i.e. mtk_core_boot_epoch() at
     * alloc time) -- NOT ctx->boot_epoch, which is the CALLING
     * TRANSPORT's own link/peer epoch (e.g. native SPI v1's own
     * dctx->boot_epoch, independently reset on a peer HELLO -- see
     * mtek_spi_native_dispatch.c's own epoch-reset fix). These are two
     * genuinely independent epoch concepts; mtk_op_snapshot validates
     * against the former, and passing the latter here would make every
     * snapshot lookup spuriously fail (mismatched epoch => "not found"
     * => op_is_terminal_now's own fail-safe treats it as already
     * terminal), silently skipping this operation's own send loop
     * entirely -- a real bug this session's own TSan-driven fix
     * introduced and caught before it shipped. */
    uint32_t my_token = id.token, my_epoch = id.boot_epoch;
...
deauth_loop_done:
    ...
    /* RC7 independent audit P0 "Shared operation/session state remains
     * data-racy": "STOP and worker paths can both restore the radio,
     * release the arbiter, and emit lifecycle output." Whichever of this
     * natural loop-exit and a concurrent handle_deauth_stop call (on a
     * different worker/task) wins deauth_finalize's own mtk_op_claim_
     * finalization is the ONLY one that performs cleanup below. Losing
     * this race means handle_deauth_stop already did every one of these
     * exact steps -- doing them again here would double-release the
     * arbiter (potentially releasing a DIFFERENT operation's now-active
     * lease), double-restore the radio, and double-emit DEAUTH_STOPPED to
     * a real peer. */
    {
        uint8_t final_status;
         ...
         * between winning and actually emitting DEAUTH_STOPPED below.
         * mtk_op_begin_publish_guard, held across the whole publish,
         * closes that window (see its own doc comment in mtek_core.h).
         * ctx is this same, still-in-scope request's own context (this
         * long-running send loop and its own finalize tail share one
         * call frame -- no separate long-lived session struct is needed
         * here to carry a session_generation the way s_hs/s_cap/s_sig/
         * s_gatt below do). */
...
static void handle_deauth_stop(mtk_request_ctx_t *ctx, const mtk_opcode_entry_t *op,
    ...
    /* RC12 item 1: family gate (DEAUTH_START) -- a non-DEAUTH token is
     * rejected NOT_FOUND before deauth_finalize could ever claim, clean up,
     * or transition a foreign operation. */
     ...
     * record pointer reads with atomic snapshots": a fresh locked snapshot
     * (not a raw rec->state/final_status read) is the only honest way to
     * report the operation's real current state -- deauth_finalize may
     * have lost the claim race to a concurrent natural loop-exit that has
     * not yet finished writing the truthful terminal status. */
    mtk_operation_record_t snap;
...
static void handle_deauth_status(mtk_request_ctx_t *ctx, const mtk_opcode_entry_t *op,
    ...
    mtk_operation_record_t snap;
    if (!mtk_op_snapshot_family(req.operation_token, ctx->boot_epoch, WIFI_SERVICE_ID, DEAUTH_START_OPCODE, &snap)) { respond_empty(ctx, MTK_STATUS_NOT_FOUND); return; }
    /* RC5 independent audit P1: real live progress for a still-RUNNING
     * operation (was always 0), matched by token identity against the
     * single session-scoped s_deauth (the arbiter's single-active-D-class
     * model means at most one deauth operation is ever RUNNING, so a
     * stale s_deauth from a prior, now-terminal operation is never
     * misreported here). */
...
typedef struct {
    ...
    mtk_sink_t sink;
    /* RC7 independent audit P0 "Shared operation/session state remains
     * data-racy": TOKEN, not a retained `mtk_operation_record_t *` -- see
     * s_deauth's own matching doc comment above for the full ABA-hazard
     * rationale (the stakes are higher here: this session's own buf/len are read by
     * handle_handshake_status/_read keyed only on the CALLER's requested
     * token matching a fresh mtk_op_find, with no check at all that it
     * still refers to THIS session before this fix -- a stale/reused slot
     * could otherwise return one operation's captured bytes attributed to
     * a completely different one). hs_frame_cb re-resolves this to a live
     * record via mtk_op_find each time it needs one, rather than holding
     ...
     * ORIGINALLY admitted under (mtk_request_ctx_t's own field, 0 for
     * every non-native-SPI adapter -- never fenced), captured once at
     * handle_handshake_start time, alongside token/boot_epoch/sink. Both
     * hs_frame_cb (a genuine background driver callback that can fire
     * long after the request that armed it has returned) and
     * handshake_finish read THIS stored value -- never a caller's own
     * ctx->session_generation, which for hs_frame_cb does not even exist
     * (no request context at all) and for handle_handshake_stop would be
     * the STOP caller's own session, not this long-lived session's
     * originating one. */
     ...
     * the requested AP/station": the target AP's BSSID from this
     * session's own HANDSHAKE_START request -- without this, any EAPOL-
     * Key frame from ANY nearby network sharing the same channel would
     * be accepted into this capture, not just the one the caller asked
     * for. */
...
/* RC7 independent audit item 11 "EAPOL frames are not filtered to the
 * requested AP/station": true if any of the frame's three 802.11 address
 * fields (addr1/addr2/addr3 -- the same from/to/BSSID convention already
 * established for MonstaShark's own BSSID filter, mtek_capture_logic.c's
 * frame_matches_filter) name the target BSSID, so a 4-way handshake
 * exchanged between the target AP and ANY of its associated stations is
 * captured, not just traffic addressed exactly one specific direction. */
...
/* RC7 independent audit P0 "Shared operation/session state remains
 * data-racy" + item 11 "Handshake capture is unsafe and semantically
 * incomplete" ("natural success marks the operation complete but
 * normally does not stop promiscuous mode, release the arbiter, restore
 * STA, or emit the terminal stopped event"): a real target's M4
 * completion (hs_frame_cb, firing from the Wi-Fi driver's own task) and
 * an explicit STOP request (handle_handshake_stop, firing from a
 * transport dispatch) can now genuinely race each other -- both call
 * this ONE shared finish routine with their own desired final state;
 * mtk_op_transition's return value (not a separate racy pre-check)
 * decides which of them actually performs cleanup, so it happens exactly
 * once regardless of which side reaches it first. Previously, M4
 * completion only transitioned the operation record and did nothing
 * else -- leaving the radio in promiscuous mode indefinitely on a real
 * target until a client happened to also send an explicit STOP. */
/* RC11 independent correction order P0 "operation finalization is not
 * exactly-once/truthful": this used to call mtk_op_transition FIRST
 * (locking in `status` as the record's own final_status before restore's
 * real result was even known -- a STATUS/STOP query racing this could
 * observe a stale "OK" the emitted event's own separately-computed field
 * simultaneously contradicted) and released the arbiter unconditionally
 * regardless of restore's own result (mtek_wifi_restore_and_release's own
 * doc comment: "Failure quarantines the radio behind the existing lease"
 * -- releasing anyway here defeated that entirely for this operation).
 * Now: claim the single finalization path first (mtk_op_claim_
 * finalization -- matching mtek_capture_service.h's own established
 * capture_teardown pattern exactly), perform the real cleanup, THEN write
 * the one truthful terminal state/status derived from what cleanup
 * actually found -- never the other order. Takes (token, boot_epoch), not
 * a retained pointer: hs_frame_cb's own `rec` is a fresh, same-callframe
 * mtk_op_find result (never retained past this call either way), but
 * expressing this function's own contract in terms of the atomic
 * token/epoch pair -- like mtek_capture_service.h's own capture_teardown
 * -- keeps every caller symmetric and never requires a live pointer. */
static void handshake_finish(uint32_t token, uint32_t boot_epoch, mtk_op_state_t final_state, uint8_t status) {
    ...
    /* RC8 independent audit P0-3: snapshot the shared fields this event
     * needs under the lock, then use/emit AFTER releasing it -- never
     ...
     * brand-new handle_handshake_start could race in, acquire MTK_ARB_H,
     * and reinitialize s_hs for a completely different session; reading
     * s_hs's own fields (session_generation/sink/len) AFTER that point
     * could pick up the NEW session's values while still reporting THIS
     * (old) token -- exactly the "old callback must never read/modify a
     * newly initialized session's state" hazard this round closes.
     * session_generation is the ORIGINATING session's own, stored once at
     * handle_handshake_start time -- never a caller's own ctx, which this
     * function does not even have: it is called from hs_frame_cb, a
     * background driver callback with no request context at all, and
     * from handle_handshake_stop, whose own STOP-caller session is not
     * necessarily this long-lived session's originating one. */
    wifi_lock();
    ...
    /* RC10 independent correction order P0 "restore is not actually
     * failure-atomic" + RC11 "keep the radio lease quarantined, never
     * report/expose it as healthy/free on a restore failure":
     * mtek_wifi_restore_and_release only releases MTK_ARB_H if restore
     * itself actually succeeded; a failure retains the lease (quarantined,
     * mtek_wifi_radio_is_quarantined) rather than freeing it for a new
     * operation to acquire over unconfirmed radio state. */
...
static void hs_frame_cb(void *user, const uint8_t *frame, uint16_t len, int8_t rssi, uint8_t channel) {
     ...
     * own promisc_start call and frozen inside this exact registration's
     * own callback parameter for its whole lifetime -- never a pointer to
     * the mutable s_hs global (the previous design: `user == &s_hs`,
     * which is no different from reading s_hs directly and carries no
     * identity of its own at all). esp32_promisc_service can legitimately
     * copy (cb, user) together, under its own mutex, an instant BEFORE a
     * concurrent STOP runs -- and, on a real target, a brand-new
     ...
     * executes. Reading s_hs.token/boot_epoch through a live pointer (the
     * old design) would validate this stale call against the NEW
     * session's own identity and could corrupt or even complete it using
     * frame bytes that actually came from the OLD session's radio
     * capture. Comparing this frozen token against s_hs's CURRENT token,
     * under lock, is the first and only gate that cannot be fooled this
     * way: a mismatch means s_hs has moved on, regardless of how self-
     * consistent whatever it currently holds looks. */
     ...
     * could have been reinitialized for a brand-new handshake session in
     * the meantime. Re-validated HERE, under the same lock protecting
     * every field this call is about to mutate, immediately before
     * mutating any of them: a mismatch means this stale callback
     * invocation must not touch whatever session s_hs now holds. */
     ...
     * criteria." The minimum genuinely crackable capture needs BOTH the
     * AP's ANonce (M1) and the station's SNonce+MIC (M2) -- an offline
     * crack attempt is mathematically impossible with only one of them,
     * regardless of whether M4 was also observed. 0x03 = bit0 (M1) AND
     * bit1 (M2) both set (was `& 0x03` != 0, true if EITHER bit was set
     * -- the bug). M3 is not required for cracking and is not checked
     * here; M4 remains the trigger (its own SUCCESS phase/event), now
     * gated on the real prerequisite. */
...
static void handle_handshake_start(mtk_request_ctx_t *ctx, const mtk_opcode_entry_t *op,
     ...
     * contexts with no request ctx of their own -- can validate against
     * it later. */
     ...
     * selection, or promiscuous-mode enable failed on the real HAL, the
     * operation stayed RUNNING forever (no cleanup, no terminal event)
     * and the initiating deauth burst below still fired regardless,
     * transmitting real frames for a capture that could never actually
     * receive anything. Closed exactly like hs_frame_cb's own M4
     * completion and handle_handshake_stop: one shared handshake_finish
     * call, gated on mtk_op_transition's own return value (a concurrent
     * STOP dispatched in the brief window between ACCEPTED and this
     * failure check is already handled the same way every other race in
     * this file is), so cleanup/restore/the terminal event each happen
     * exactly once. */
    /* P0 correction (Round 8, item 3): `user` is this exact registration's
     * own immutable token, not a pointer to the mutable s_hs global -- see
     * hs_frame_cb's own doc comment for the full ABA-hazard rationale. */
...
static void handle_handshake_status(mtk_request_ctx_t *ctx, const mtk_opcode_entry_t *op,
    ...
    /* RC7 independent audit P0 "Shared operation/session state remains
     * data-racy": s_hs's own captured bytes belong to whichever token
     * s_hs.token currently names -- a valid-but-unrelated token (a stale
     * slot address reused by a different, later operation) must never be
     * answered with a PRIOR session's leftover length. */
...
static void handle_handshake_read(mtk_request_ctx_t *ctx, const mtk_opcode_entry_t *op,
    ...
    /* Same stale-session guard as handle_handshake_status above -- never
     * hand back a PRIOR session's captured bytes for an unrelated token.
     * RC8 independent audit P0-3: locked for the whole read (including
     * the buf memcpy below) -- hs_frame_cb can be concurrently appending
     * to s_hs.buf/len on a real target's own Wi-Fi driver task. */
    wifi_lock();
...
static void handle_handshake_stop(mtk_request_ctx_t *ctx, const mtk_opcode_entry_t *op,
    ...
    if (mtk_decode(op->req_desc, &req, req_bytes, req_len, NULL) != MTK_CODEC_OK) { respond_empty(ctx, MTK_STATUS_PROTOCOL_ERROR); return; }
    mtk_operation_record_t precheck_snap;
    /* RC12 item 1: family gate (HANDSHAKE_START) -- a non-HANDSHAKE token
     * is rejected NOT_FOUND before handshake_finish could finalize a
     * foreign operation. */
    if (!mtk_op_snapshot_family(req.operation_token, ctx->boot_epoch, WIFI_SERVICE_ID, HANDSHAKE_START_OPCODE, &precheck_snap)) { respond_empty(ctx, MTK_STATUS_NOT_FOUND); return; }
    ...
    /* RC11 independent correction order P0 "replace unsafe operation-
     * record pointer reads with atomic snapshots": handshake_finish no
     * longer writes rec->state/final_status directly inline with this
     * call (it claims finalization, cleans up, THEN writes the truthful
     * terminal state -- possibly after this STOP itself lost that race to
     * a concurrent hs_frame_cb) -- a fresh locked snapshot is the only
     * honest way to report the operation's real current state here,
     * matching mtek_capture_service.h's own handle_capture_stop pattern. */
    mtk_operation_record_t snap;
    if (!mtk_op_snapshot_family(req.operation_token, ctx->boot_epoch, WIFI_SERVICE_ID, HANDSHAKE_START_OPCODE, &snap)) { respond_empty(ctx, MTK_STATUS_NOT_FOUND); return; }
...
static void handle_wifi_stop_all(mtk_request_ctx_t *ctx) {
    ...
    if (active == MTK_ARB_WMC || active == MTK_ARB_WS || active == MTK_ARB_BEACON ||
         ...
         * blocking HAL scan/connect operation is still running"): WMC
         * (STA_CONNECT) and WS (AP/STA_SCAN) each drive an uncancellable
         * blocking HAL call with no cancel hook of their own reliably
         * honored within this one synchronous call -- the generic
         * "transition-then-unconditionally-restore-and-force-release"
         * path below (still correct and UNCHANGED for D/H/M/BEACON/SAP/
         * RAW, none of which have this hazard) would restore/release the
         * radio while that blocking call might still genuinely be
         * running, letting a brand-new operation of the same class start
         * a second, overlapping HAL call. These two classes are instead
         * handled exactly like mtek_wifi_cancel_active_for_peer_reset's
         * own WMC/WS handling -- fence the token (WMC) or run the
         * established quiescence handshake (WS), NEVER touching restore/
         * arbiter directly; only the real worker's own tail (handle_sta_
         * connect's/handle_ap_scan_start's/handle_sta_scan_start's own,
         * already token-ownership-gated) safely restores/releases, once
         * its blocking call actually returns. */
        if (active == MTK_ARB_WMC) {
             ...
             * verified recovery before releasing ownership": a single best-
             * effort restore_sta_mode call followed by an UNCONDITIONAL
             * mtk_arbiter_force_release used to hand the radio to the next
             * caller as confirmed-free even when restore itself reported
             * failure -- exactly the "report/expose a quarantined lease as
             * healthy/free" this correction forbids. esp32_restore_sta_mode's
             * own contract (mtek_wifi_hal_esp32.c) preserves the captured
             * prior-state snapshot across a failed attempt specifically so a
             * retry here can attempt the exact same recovery again, rather
             * than acting on a half-restored, now-unknown state; a bounded
             * number of attempts (with a real pacing delay between them, via
             * the same s_hal->pace_delay_ms every other paced retry in this
             * file already uses) gives a transient failure a real chance to
             * clear before this falls back to quarantine. The arbiter lease
             * is force-released only once a retry actually confirms success;
             * if every attempt fails, the lease stays held (quarantined) so a
             * later operation cannot mistake it for free -- the caller may
             * call WIFI_STOP_ALL again to retry. This response schema has no
             * per-item restore-status field to extend without touching the
             * frozen wire format; the honest recovery signal is
             * GET_WIFI_RECOVERY_STATE's own sta_mode_restored field, which
             * already reflects "is the arbiter free" truthfully once this is
             * fixed. */
            int restore_rc = -1;
 ...
 * handshake_finish) so cleanup is byte-for-byte identical to a real STOP
 * -- never a second, independently-drifting cleanup path -- and falls
 * back to the same transition+restore shape handle_wifi_stop_all's own
 * WS branch already uses for AP/STA scan (which have no single shared
 * finalize helper of their own, only STOP's own bounded quiescence
 * handshake, unsuitable for a synchronous cancel-and-evict). */
mtk_op_id_t mtek_wifi_cancel_active_for_peer_reset(void) {
     ...
     * already released the instant the original CONNECT attempt
     * completed (this file's own established design: WMC is held only
     * for the duration of the attempt itself, never while merely
     * remaining connected -- see handle_sta_connect). Checked and torn
     * down here independently of the arbiter-class switch below, which
     * only ever sees an IN-PROGRESS attempt, never an already-connected,
     * arbiter-free session. */
    ...
    switch (active) {
        ...
        case MTK_ARB_WMC:
             ...
             * (mtek_wifi_hal.h has none) -- fence instead of waiting:
             * claim this exact token via the SAME token-based transition
             * handle_sta_connect's own tail uses, so only the winner ever
             * touches s_sta_connected. A worker that later loses this race
             * (per handle_sta_connect's own doc comment) tears down any
             ...
             * to has no cancel hook and may still genuinely be running on
             * another thread; releasing the class here would let a
             * brand-new STA_CONNECT acquire it and start a SECOND,
             * overlapping connect() against the same radio hardware.
             * handle_sta_connect's own tail already releases MTK_ARB_WMC
             * unconditionally-but-atomically-ownership-checked
             * (mtk_arbiter_release_if_owner) the instant its real HAL
             * call returns, regardless of whether it also wins this same
             * transition race -- that is the ONLY place this class is
             * ever safely released for an in-progress connect, exactly
             * once, never orphaned and never released early. */
            mtk_op_transition_by_token(tok, epoch, MTK_OPS_STOPPED, MTK_STATUS_OK, now_ms());
            break;
        case MTK_ARB_WS: {
            /* Mirror handle_ap_scan_stop/handle_sta_scan_stop's own
             * established quiescence handshake EXACTLY: signal
             * cancellation (safe no-op if nothing is in flight for
             * either), then wait BOUNDED for the worker's own natural
             * completion path (handle_ap_scan_start/handle_sta_scan_
             * start, both already gated on mtk_op_claim_finalization) to
             * perform the REAL cleanup itself -- radio restore, arbiter
             * release, truthful transition, terminal event -- never
             * touch the radio directly here, which would race that same
             * worker's own concurrent HAL access to it. This whole
             * invalidation runs synchronously while processing the
             * current HELLO, strictly before any later request can be
             * fed, so whatever the worker publishes while this call
             * waits is safely serialized before any new peer session's
             * own operation could possibly start. This never touches the
             * radio itself (never restore_sta_mode, never mtk_arbiter_
             * release) -- that remains solely handle_ap_scan_start's/
             * handle_sta_scan_start's own job, now performed based on
             * arbiter TOKEN ownership rather than solely on winning the
             * op-table claim (see those handlers' own doc comments),
             * specifically so the forced timeout finalization below can
             ...
             * itself must not be left queryable/RUNNING as a live "old
             * session" operation (it would otherwise survive the coming
             * mtk_op_evict_all_terminal() sweep untouched, indefinitely
             * answerable to a new peer session's own STATUS/STOP calls
             * against the same token number). Force-claim and transition
             * it to a terminal state right now so it becomes immediately
             * evictable -- deliberately WITHOUT touching the arbiter
             * class, which must stay held exactly as long as the radio
             * is genuinely busy. When the worker's own blocking call
             * eventually returns, it will lose this same claim (mtk_op_
             * claim_finalization returns false for it) and correctly
             * skip publishing any state/event into the new session, while
...
static void handle_raw_tx_send(mtk_request_ctx_t *ctx, const mtk_opcode_entry_t *op,
     ...
     * channel but does not select it." req.channel was previously decoded
     * and range-checked, then never used at all -- the frame transmitted
     * on whatever channel the radio already happened to be on. Selected
     * and VERIFIED here before transmit; never transmits on a channel
     * selection failure. */
...
static void mtek_wifi_dispatch(mtk_request_ctx_t *ctx, const mtk_opcode_entry_t *op,
    ...
    switch (op->opcode) {
         ...
         * boundary): community-lineage attack conveniences, not shipped
        ...
        case 0x0022: case 0x0023: case 0x0024: case 0x0025: respond_empty(ctx, MTK_STATUS_UNSUPPORTED); return; /* CAPTIVE_PORTAL_* */
#endif
