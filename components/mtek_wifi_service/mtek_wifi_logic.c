...
typedef struct {
    /* This used to store `mtk_request_ctx_t *ctx` -- a raw pointer into whatever
     * transient storage the dispatching adapter/router happened to hand
     * handle_handshake_start (a UART handler's own stack frame, or the router's
     * async-pool slot, which is released and reusable the instant that handler
     * returns). The real target's promiscuous-mode callback (hs_frame_cb below)
     * fires later, from the Wi-Fi driver's own task, well after that storage is
     * gone or reused for an unrelated operation -- a genuine
     * use-after-return/use-after-reuse. Storing a byte-for-byte VALUE COPY of
     * just the sink here (matching the pattern already used safely by
     * s_sig/s_gatt below) removes the dependency on the dispatching call's own
     * stack/pool-slot lifetime entirely: the function-pointer table and the
     * `user` pointer it captures are copied once, at session-start time, and
     * nothing about this struct depends on `ctx` (the argument) remaining valid
     * a moment longer. `sink.user` itself must still point to storage that
     * outlives this operation -- every adapter capable of originating an
     * operation with a delayed HAL callback (native/Mtek Compatibility SPI's
     * persistent event_queue, or the UART REPL's persistent per-session capture,
     * see mtek_uart_adapter.c) is responsible for supplying that. */
    mtk_sink_t sink;
...
static void mtek_wifi_dispatch(mtk_request_ctx_t *ctx, const mtk_opcode_entry_t *op,
    ...
    switch (op->opcode) {
        ...
        /* Optional capability modules (docs/ARCHITECTURE.md's module boundary):
         * community-lineage attack conveniences, not shipped List A /
         * required-List B canonical-core contracts. Each has its own
         * compile-time Kconfig gate (main/Kconfig.projbuild); radio-behavior
         * logic for all four is not implemented regardless of the gate
         * (docs/PROVENANCE.md) -- the explicit case labels below (present only
         * when the module is compiled in) exist so the capability manifest can
         * honestly distinguish "module compiled out" from "module compiled in,
         * not yet implemented", both of which currently answer the same wire
         * status (UNSUPPORTED, no side effect) that a DISABLED capability would. */
...
#endif
        /* SOFTAP (0x0019-0x001B) and PMKID_CAPTURE (0x001E/0x0026): SoftAP is a
         * general reusable platform capability candidate for canonical core (not
         * a module -- docs/ARCHITECTURE.md), and PMKID_CAPTURE has no Mtek
         * Compatibility/M1-Compatibility mapping at all per the accepted
         * contract; neither is module-gated. Both, plus anything else reaching
         * this point, are not implemented -- see docs/PROVENANCE.md. Returns the
         * same wire status a DISABLED capability would (no side effect). */
