 ...
 * Legacy SPI Compatibility) onto the confirmed production pins, with AUTO profile
 ...
 * means the UART REPL task can now genuinely run at the same time and
 * must serialize against the exact same shared state through the exact
 * same lock). */
 ...
 * previously discarded entirely). A -1 return means the SPI/Legacy SPI Compatibility
