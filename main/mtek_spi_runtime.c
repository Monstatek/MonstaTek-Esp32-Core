 ...
 * `spi_slave` peripheral onto the production board's confirmed pins and runs the
 * AUTO-discovery + boot-exclusive dispatch loop for the native SPI v1 and Mtek
 * Compatibility/C3 SPI adapters.
 *
 * Confirmed facts this file implements against (both profiles, one shared
 * physical bus -- AUTO discovery could not work at all if the two profiles
 * disagreed on pins/mode, so a single shared spi_slave configuration is a
 * structural requirement, not a guess): - ESP32 acts as SPI SLAVE; SPI mode 1
 * (CPOL=0, CPHA=1), MSB-first. - Confirmed production pins: SCLK=GPIO7,
 * MOSI=GPIO12, MISO=GPIO13, CS=GPIO15, HANDSHAKE=GPIO14 (same doc,
 * cross-confirmed against an independent production-board pin table). - HANDSHAKE timing: driven HIGH in the SPI slave's post_setup_cb
 * once the transaction is armed and the peer may begin clocking it, and LOW in
 * post_trans_cb once the transfer has completed and the line is ready to be
 * re-armed for the next transaction -- confirmed ESP-side behavior, mirrored
 * here via spi_slave_interface_config_t's own callback hooks (same doc Sec 1.2).
 * - DATAREADY (GPIO6, per the hardware evidence and profile-bootstrap
 * feasibility records) is profile-specific, per-profile confirmed behavior: for Mtek
 * Compatibility/C3, the pinned reference build leaves this pin's ESP-side
 * driving DISABLED by default (`PIN_DATA_READY (-1)` unless a Kconfig option
 * most Mtek Compatibility builds do not set) -- HANDSHAKE alone gates clocking
 * in that confirmed default configuration, so this implementation matches it
 * (DATAREADY held low/unused whenever Mtek Compatibility is the locked profile).
 * For native SPI v1, SPI_PROTOCOL_V1.md itself specifies the semantic exactly
 * ("GPIO state machine": high when the armed MISO cell is useful or useful
 * outbound data is queued, low otherwise) -- implemented per-transaction below,
 * driven high whenever the cell about to be armed is a real RESPONSE/EVENT/etc
 * (not IDLE). - Discovery phase: 512-byte non-dispatching transactions
 * (SPI_PROTOCOL_V1.md); a valid Mtek Compatibility/C3 frame or native HELLO
 * locks that profile for the remainder of the boot session; the losing adapter
 * never dispatches again until reset (same doc, "Runtime transport selection").
 * Cross-profile rejection and the discovery parse itself reuse
 * mtek_transport_select, already exhaustively tested on host
 * (test_transport_select.c) -- not reimplemented here. - Once locked, native
 * negotiates up to the 1024-byte steady-state cell (same doc); Mtek
 * Compatibility's cell size is always the fixed 512 bytes (mtek_compat_frame.h).
 * - Clock rate is driven by the SPI *master* (the STM32/M1 side) in this slave
 * role; the confirmed 4.6875 MHz figure only bounds what the master should
 * assert, and is out of this slave-mode driver's own configuration surface
 * (spi_slave_interface_config_t carries no clock-rate field -- there is nothing
 * to guess here). */
...
static void spi_output_notify(void *task) { xTaskNotifyGive((TaskHandle_t)task); }
#endif
...
static void IRAM_ATTR spi_post_trans_cb(spi_slave_transaction_t *t) {
    ...
    vTaskNotifyGiveFromISR((TaskHandle_t)t->user, &wake);
...
static int configure_gpios(void) {
    ...
    /* DATAREADY: initialized low. For the Mtek Compatibility/C3 profile it is left at
...
static void spi_runtime_task(void *arg) {
    ...
    size_t cell_size = MTK_COMPAT_CELL_SIZE; /* 512, the shared discovery-phase size */
    /* Set true the moment this SPI runtime wins the cross-transport claim
     * (mtk_transport_claim_try, right after `profile` first locks below) --
     * gates every actual dispatch call for the rest of this boot session. Before
     * that (still MTK_TRANSPORT_AUTO) or if a different adapter (the UART REPL
     * task) already won, this stays 0 and both the native and Mtek
     * Compatibility/C3 branches below answer well-formed IDLE only, never
     * reaching mtk_router_dispatch -- see each branch's own comment. */
    int can_dispatch = 0;

#if CONFIG_MTEK_ADAPTER_COMPAT_C3
    /* compat_dctx (~12KB, mostly compat_dctx.outbound's 4082-byte reassembly
     * buffer plus token/ generation bookkeeping) is `static`, not stack-local.
     * This task never returns (infinite loop below) for the whole boot session,
     * so static storage has exactly the same lifetime a stack-local here would
     * have needed anyway -- the difference is it no longer counts against
     * spi_runtime_task's configured stack size at all. A background worker task
     * holding a pointer into it (via compat_dctx.event_queue) is exactly the
     * persistent-adapter-owned- queue shape the router's async SAFETY CONTRACT
     * requires, and static storage satisfies that at least as well as a
     * never-returning stack frame did. */
    static mtk_compat_dispatch_ctx_t compat_dctx;
    mtek_compat_dispatch_init(&compat_dctx, mtk_core_boot_epoch());
    mtk_async_queue_set_lock(&compat_dctx.event_queue, queue_lock, queue_unlock, shared_mutex);
#endif
#if CONFIG_MTEK_ADAPTER_NATIVE_SPI
    ...
    mtk_async_queue_set_notify(&native_dctx.event_queue, spi_output_notify, xTaskGetCurrentTaskHandle());
    int armed_useful = 0;
#endif
...
retry_transaction: ;
        ...
        t.tx_buffer = txbuf;
        t.rx_buffer = rxbuf;
        t.user = xTaskGetCurrentTaskHandle();
        err = spi_slave_queue_trans(MTK_SPI_HOST, &t, portMAX_DELAY);
        if (err != ESP_OK) {
            ESP_LOGW(TAG, "spi_slave_queue_trans error %d", (int)err);
            vTaskDelay((TickType_t)MTK_CLAMP_MIN_ONE_TICK(pdMS_TO_TICKS(10)));
            goto retry_transaction;
        }

        /* A timeout waiting for completion does NOT return DMA ownership.
         * Keep the same descriptor and buffers armed until get_trans_result
         * succeeds. Wakeups from output producers and the completion ISR are
         * hints; queue/readiness state is always re-read. A bounded sleep also
         * services elapsed-time expiry while the SPI master is silent. */
        spi_slave_transaction_t *completed = NULL;
        esp_err_t last_wait_error = ESP_OK;
        for (;;) {
#if CONFIG_MTEK_ADAPTER_NATIVE_SPI
            mtek_spi_native_dispatch_tick(&native_dctx, (uint32_t)(esp_timer_get_time() / 1000));
            int ready = profile == MTK_TRANSPORT_NATIVE_SPI && can_dispatch &&
                        (armed_useful || native_dctx.outbound.active ||
                         mtk_async_queue_count(&native_dctx.event_queue) != 0);
            gpio_set_level(PIN_DATAREADY, ready);
#endif
            err = spi_slave_get_trans_result(MTK_SPI_HOST, &completed, 0);
            if (err == ESP_OK) break;
            if (err != ESP_ERR_TIMEOUT && err != last_wait_error)
                ESP_LOGW(TAG, "spi_slave_get_trans_result error %d; retaining DMA ownership", (int)err);
            last_wait_error = err;
            ulTaskNotifyTake(pdTRUE, (TickType_t)MTK_CLAMP_MIN_ONE_TICK(pdMS_TO_TICKS(10)));
        }
        /* Fixed-size physical cells, including the 512-byte HELLO_ACK phase.
         * A short/non-byte-aligned transfer must not consume stale RX bytes,
         * discard the pending TX reply, or advance cell-size negotiation. */
        if (t.trans_len != cell_size * 8) {
        ...
        if (profile == MTK_TRANSPORT_AUTO) {
            ...
            } else if (profile == MTK_TRANSPORT_COMPAT_C3_SPI) {
                ESP_LOGI(TAG, "AUTO discovery locked: Mtek Compatibility/C3 SPI");
            }
            ...
            /* The SPI-bus- internal profile lock above (native vs. Mtek
             * Compatibility/C3) is a separate question from the CROSS-transport
             * race against the UART REPL task -- this is that race's own
             * attempt, made exactly once, right when this bus first recognizes
             * either profile. If a different adapter already won (the UART REPL
             * task saw a genuine command line first), can_dispatch stays 0 for
             * the rest of this boot session: this loop keeps running the real
             * wire-level state machine (HELLO_ACK, cell-size negotiation) so the
             * peer on this physical bus is never left hanging mid-handshake, but
             * every dispatch branch below only ever answers IDLE once locked-out
            ...
            if (profile == MTK_TRANSPORT_NATIVE_SPI) {
                ...
                can_dispatch = mtk_transport_claim_try(MTK_PUBLIC_ADAPTER_COMPAT_C3_SPI);
            }
        }

#if CONFIG_MTEK_ADAPTER_COMPAT_C3
        if (profile == MTK_TRANSPORT_COMPAT_C3_SPI) {
            ...
            if (!can_dispatch) {
                ...
                mtk_compat_build_cell(&resp_hdr, NULL, txbuf);
            } else {
            mtk_compat_header_t bhdr; const uint8_t *bpayload;
            mtk_compat_parse_result_t pr = mtk_compat_parse_bounded(rxbuf, received_bytes, &bhdr, &bpayload);
            if (pr == MTK_COMPAT_PARSE_OK && bhdr.msg_type == MTK_COMPAT_MSG_REQ) {
                mtek_compat_dispatch_request(&compat_dctx, &bhdr, bpayload, &resp_hdr, resp_payload, &resp_len);
                mtk_compat_build_cell(&resp_hdr, resp_payload, txbuf);
            } else if (pr == MTK_COMPAT_PARSE_OK && bhdr.msg_type == MTK_COMPAT_MSG_IDLE) {
                /* Peer polling: drain a staged fragmented response if any
                 * is outstanding, else a well-formed IDLE. */
                mtek_compat_dispatch_poll_outbound(&compat_dctx, &resp_hdr, resp_payload, &resp_len);
                mtk_compat_build_cell(&resp_hdr, resp_payload, txbuf);
            } else {
                ...
                mtk_compat_header_t idle_hdr;
                memset(&idle_hdr, 0, sizeof(idle_hdr));
                idle_hdr.magic = MTK_COMPAT_MAGIC; idle_hdr.version = MTK_COMPAT_VERSION; idle_hdr.msg_type = MTK_COMPAT_MSG_IDLE;
                mtk_compat_build_cell(&idle_hdr, NULL, txbuf);
            }
            }
        } else
#endif
        ...
        if (profile == MTK_TRANSPORT_NATIVE_SPI) {
            ...
            if (!can_dispatch) {
                /* Same cross-transport lockout as the Mtek Compatibility/C3 branch
            ...
            if (pr == MTK_SPI_PARSE_OK && nhdr.msg_class == MTK_SPI_CLASS_IDLE) {
                ...
                mtek_spi_native_dispatch_feed_cell(&native_dctx, &nhdr, npayload,
             ...
             * transaction about to be armed (the next spi_slave_queue_trans
             * call below). */
            armed_useful = resp_hdr.msg_class != MTK_SPI_CLASS_IDLE;
            gpio_set_level(PIN_DATAREADY, can_dispatch &&
                (armed_useful || native_dctx.outbound.active || mtk_async_queue_count(&native_dctx.event_queue) != 0));
        } else
#endif
        {
...
int mtek_spi_runtime_start(SemaphoreHandle_t shared_mutex) {
     ...
     * per-call-frame depth the native/Mtek Compatibility dispatch call chain and
