 ...
 * SPI v1 and Legacy SPI Compatibility adapters.
 *
 ...
 *    Legacy SPI Compatibility m1_link.c `#define SPI_MODE 1` and its header comment).
 ...
 *    behavior: for Legacy SPI Compatibility, the pinned reference build leaves this
 *    pin's ESP-side driving DISABLED by default (`PIN_DATA_READY (-1)`
 *    unless a Kconfig option most Legacy SPI Compatibility builds do not set) -- HANDSHAKE
 ...
 *    Legacy SPI Compatibility is the locked profile). For native SPI v1, SPI_PROTOCOL_V1.md
 ...
 *    (SPI_PROTOCOL_V1.md); a valid Legacy SPI Compatibility frame or native HELLO locks
 ...
 *    cell (same doc); Legacy SPI Compatibility's cell size is always the fixed 512 bytes
 *    (mtek_compat_frame.h).
...
static int configure_gpios(void) {
    ...
    /* DATAREADY: initialized low. For the Legacy SPI Compatibility profile it is left at
...
static void spi_runtime_task(void *arg) {
     ...
     * already won, this stays 0 and both the native and Legacy SPI Compatibility branches
     * below answer well-formed IDLE only, never reaching
     * mtk_router_dispatch -- see each branch's own comment. */
    int can_dispatch = 0;

#if CONFIG_MTEK_ADAPTER_COMPAT_SPI
...
retry_transaction: ;
        ...
        if (profile == MTK_TRANSPORT_AUTO) {
             ...
             * internal profile lock above (native vs. Legacy SPI Compatibility) is a
            ...
            if (profile == MTK_TRANSPORT_NATIVE_SPI) {
                ...
                can_dispatch = mtk_transport_claim_try(MTK_PUBLIC_ADAPTER_COMPAT_SPI);
            }
        }
        ...
        if (profile == MTK_TRANSPORT_NATIVE_SPI) {
            ...
            if (!can_dispatch) {
                /* Same cross-transport lockout as the Legacy SPI Compatibility branch
