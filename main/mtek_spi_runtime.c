 ...
 * SPI v1 and Mtek Compatibility/C3 SPI adapters.
 *
 ...
 *    Mtek Compatibility m1_link.c `#define SPI_MODE 1` and its header comment).
 ...
 *    behavior: for Mtek Compatibility/C3, the pinned reference build leaves this
 *    pin's ESP-side driving DISABLED by default (`PIN_DATA_READY (-1)`
 *    unless a Kconfig option most Mtek Compatibility builds do not set) -- HANDSHAKE
 ...
 *    Mtek Compatibility is the locked profile). For native SPI v1, SPI_PROTOCOL_V1.md
 ...
 *    (SPI_PROTOCOL_V1.md); a valid Mtek Compatibility/C3 frame or native HELLO locks
 ...
 *    cell (same doc); Mtek Compatibility's cell size is always the fixed 512 bytes
 *    (mtek_compat_frame.h).
...
static void spi_runtime_task(void *arg) {
     ...
     * already won, this stays 0 and both the native and Mtek Compatibility/C3 branches
     ...
     * larger than the configured stacks": compat_dctx (~12KB, mostly
     * compat_dctx.outbound's 4082-byte reassembly buffer plus token/
     ...
     * compat_dctx.event_queue) is exactly the persistent-adapter-owned-
...
retry_transaction: ;
        ...
        if (profile == MTK_TRANSPORT_AUTO) {
             ...
             * internal profile lock above (native vs. Mtek Compatibility/C3) is a
